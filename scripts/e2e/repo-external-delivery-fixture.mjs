...
export async function prepareExternalRepositoryFixture(home, patchPath, env, { sessionId, workspace, objective, source }) {
  ...
  const authority = withBrokerGrantDigest({ protocol: 'assistant-actions/external-github-grant/v1', id: grantId, revision: 1, clientKeyId: 'host',
    ...
    maxActions: 8, maxTotalBytes: 1_048_576, maxCostUnits: 20, allowedOperations: ['commit', 'inspect', 'pull-request'], allowedInspectKinds: ['repository', 'branch', 'file'],
    verifiedDelivery: { ownerRouteId: admissionId, budgetId: `${admissionId}-runs` }, source: { classification: 'internal', provenanceDigest: 'b'.repeat(64) }, policyEpoch: 1, emergencyEpoch: 0 })
  ...
  const core = new ExternalBrokerCore({ instanceId: 'broker', statePath: join(actionsRoot, 'state.sqlite'), grants: [authority], policyEpoch: 1,
    ...
    inspect: async input => {
      ...
      if (input.kind === 'branch') return { observed: { name: 'automation/fix', commit: { sha: initialOid }, untrusted: true } }
  ...
  // The broker's SO_PEERCRED hook is fixture-supplied in this same-UID runner;
  // production Host peer probing remains exercised by the packaged client.
  ...
  const server = await startGitHubBrokerServer({ core, actionSocketPath, adminSocketPath, instanceId: 'broker', generation: core.snapshot().generation,
    serverPrivateKey: serverKeys.privateKey, clientPublicKey: clientKeys.publicKey, clientKeyId: 'host', adminPublicKey: adminKeys.publicKey, adminKeyId: 'admin', inspectPeerCredentials: peer,
  ...
  try {
    ...
    return { repository: 'fixture/orders', baseBranch: 'main', branch: 'automation/fix', paths: ['summarize.mjs'], externalGrantId: grantId, expiresAt, maxActions: 8, maxTotalBytes: 1_048_576, openPullRequest: true, close: () => server.stop() }
