// Anonymous real-registry proof with disposable test authorities. Never publishes.
...
import { Ed25519SourceReleaseAuthority, Ed25519SourcePublishReconciliationAuthority, invokeSourceReleaseAdapter,
  invokeSourcePublishReconciliationAdapter, parseSourceReleaseRequest, parseSourcePublishReconciliationRequest,
...
if (process.env.DSH_NPM_VERIFIER_LIVE !== '1') throw new Error('Set DSH_NPM_VERIFIER_LIVE=1 for anonymous read-only npm verification')
const args = process.argv.slice(2)
if (args.length !== 2 || args[0] !== '--output') throw new Error('Usage: npm-release-verifier-readback.mjs --output <new evidence.json>')
...
const paths = ['plugins/plugin-control-plane/src/registry-fetch.ts', 'plugins/plugin-control-plane/lib/registry-fetch.js',
  ...
  'plugins/plugin-control-plane/bin/dsh-npm-registry-adapter.js', 'scripts/e2e/npm-release-verifier-readback.mjs']
...
const evidence = { schemaVersion: 1, kind: 'npm-source-release-verifier-readback', baseCommit, startedAt: new Date().toISOString(), runtimeDigests,
  ...
  authority: 'anonymous HTTPS reads with disposable owner/signer/verifier test identities', passed: false,
  limits: ['The authorization, signed artifact statement, SBOM and provenance are readback fixtures; they do not prove a real approved build or publication.',
    'Only the registry metadata and tarball bytes are real external observations; historical expected integrity comes from committed release evidence.',
...
try {
  ...
  const unsigned = { schemaVersion: 1, kind: 'dsh-source-release-authorization', authorizationId: 'readback-authorization',
    ...
    releasePolicy: { targetBranch: 'dev', candidateId: artifact.candidateId, packageName: artifact.packageName, packageVersion: artifact.packageVersion,
      ...
      catalogId: 'readback-catalog', catalogPath: join(root, 'catalog.json'), minimumReproducibleBuilds: 2 }, authorizedAt: now - 1_000, expiresAt: now + 120_000 }
  ...
  const reconciliationRequest = parseSourcePublishReconciliationRequest({ ...common, operationId: 'live-publish-reconciliation', kind: 'dsh-source-publish-reconciliation-request',
    ambiguousPublish: { operationId: 'publish-fixture', receiptId: 'publish-receipt-fixture', receiptDigest: sha('publish-receipt-fixture'), evidenceDigest: sha('publish-ambiguity-fixture') },
    ...
    expectedArtifactStatementDigest: sourceArtifactStatementDigest(artifact), expectedArtifactSignatureDigest: sha(Buffer.from(artifactSignature, 'base64')), expectedRegistryReference: fetched.reference })
  const reconciliationReceipt = await invokeSourcePublishReconciliationAdapter(trust, reconciliationRequest)
  await new Ed25519SourcePublishReconciliationAuthority(publicKey, config.authority, config.keyId).verify(reconciliationReceipt, plan, reconciliationRequest)
  if (receipt.outcome !== 'passed' || reconciliationReceipt.evidence.outcome !== 'exists-match') throw new Error('Real npm verifier did not establish the expected exact match')
  evidence.verifierPublicKey = publicKey; evidence.ownerPublicKey = owner.publicKey.export({ type: 'spki', format: 'pem' }).toString()
  evidence.artifactSignerPublicKey = signer.publicKey.export({ type: 'spki', format: 'pem' }).toString()
  evidence.release = { request, receipt }; evidence.reconciliation = { request: reconciliationRequest, receipt: reconciliationReceipt }
