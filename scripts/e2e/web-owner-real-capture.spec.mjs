...
test('ordinary owner request captures an accepted workflow and reuses it after restart', async ({ page, context }, testInfo) => {
  ...
  const env = { ...process.env, CI: 'true', DSH_HOME: home, DSH_REPO_AUTONOMY_OBSERVER_LOG: modelLog,
    DSH_REPO_AUTONOMY_MAX_CALLS: '24', DSH_REPO_AUTONOMY_DURATION_MS: '360000' }
  ...
  let host, activePage = page, starts = 0, failed = false, sessionId, sourceAudit
  ...
  const approve = async () => {
    ...
    if (!event) {
      ...
      if (!parent || parent.data.name !== 'skill_trial') throw new Error('approval has no matching direct call or skill trial parent')
      ...
      const active = runs.filter(value => value.state === 'running' && value.sessionId === sessionId
        && value.goalId === args.goal_id && value.candidateId === args.candidate_id && value.invocationId === args.invocation_id)
      ...
      const row = query(skillsPath, 'SELECT candidate_json FROM skill_candidates WHERE id = ?', args.candidate_id)[0]
      const candidate = row && JSON.parse(row.candidate_json)
      if (!candidate || candidate.state !== 'pending' || candidate.definition.source.scope.workspace !== workspace) throw new Error('nested approval candidate is not current')
      const step = instantiate(candidate.definition, active[0].inputs).steps[Number(match[2]) - 1]
    ...
    const safeControl = ['get_goal', 'todo_write', 'goal_create', 'goal_context', 'goal_checkpoint', 'skill_capture', 'skill_captures', 'skill_status', 'skill_candidates', 'skill_trial', 'skill_activate', 'skill', 'skill_run'].includes(name)
  ...
  const wait = async (predicate, description) => {
    await expect.poll(async () => {
      ...
      if (failedRun) throw new Error(`candidate trial ${failedRun.state}: ${failedRun.steps.at(-1)?.detail ?? 'unsettled execution'}`)
  ...
  try {
    ...
    expect(query(skillsPath, 'SELECT * FROM skill_runs')).toHaveLength(1)
    ...
    expect(captured()).toHaveLength(1)
    expect(query(skillsPath, 'SELECT * FROM skill_runs')).toHaveLength(1)
    expect(query(skillsPath, 'SELECT * FROM skill_definitions')).toHaveLength(1)
    expect(query(goalsPath, 'SELECT id FROM goal_records')).toHaveLength(2)
    await stop()
    ...
    await writeFile(testInfo.outputPath('proof.json'), JSON.stringify({ capability: 'automatic-capture-and-verified-candidate-reuse', route: route.proof, hostStarts: starts, initial, capture, trial, sourceReceipt: firstGoal.receipt, replayReceipt: goalOutcome(trial.goalId).receipt, sourceHash, activation: 'explicit-owner-after-independent-trial', improvement: 'unmeasured', ordinaryRequests: true, forcedToolOrder: false, ownerReadback: true, captureHandoff: { successfulCallId: sourceHandoff.registered.data.callId, sourceNativeTurn: sourceHandoff.start.data.turn, finalNativeTurn: finalHandoff.start.data.turn } }), { mode: 0o600 })
