...
for (const permission of ['default-full', 'workspace-write']) {
  test(`fresh Web owner authenticates, streams a business goal, and resumes its session (${permission})`, async ({ page, context, playwright }, testInfo) => {
    ...
    try {
      ...
      // Keep the interactive approval proof while independently exercising the
      // shipped fresh-install default without any settings override or UI click.
      if (permission === 'workspace-write') {
        await run(join(home, 'profiles/web/node_modules/.bin/dsh-permission-setup'),
          ['--dsh-home', home, '--preset', 'workspace-write'], env)
      }
      ...
      await writeFile(testInfo.outputPath('proof.json'), JSON.stringify({
        ...
        goalScope: scope, executedTool: 'goal_create', permission,
        approvalInteraction: permission === 'workspace-write' ? 'allow-once' : 'none', initialReply,
