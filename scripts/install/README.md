...
Linux 的强隔离生命周期事务要求 Node.js、`flock`、Perl、`/usr/bin/python3` 和 bubblewrap。本地 upgrade 会先对当前 checkout 执行 `pnpm install --offline --frozen-lockfile` 和构建，任一步失败都在事务开始前终止。macOS 的 Lark/supervised upgrade 仅在整个 DSH_HOME 已静止时开放：安装器完整备份 profile，精确更新既有受管 bundle，执行组合配置与临时 Host 激活验证；失败自动恢复原 profile 并保留失败现场。该路径不重跑 onboarding、不创建或启动 LaunchAgent。

npm upgrade 则在同一生命周期锁内把发布 selector 解析为精确 cohort，在 `DSH_HOME` 外的私有目录准备最终 package manifest、workspace 配置和 lockfile；以固定的公开 registry 和 pnpm 策略完成供应链校验、完整预取，随后才创建 transaction。bwrap 内按同一份元数据执行 frozen/offline install，缓存与 store 只读，禁止 install scripts、pnpmfile hooks 和 `trustLockfile` 跳过验证。

原配置中的严格 release-age 策略保持有效；非严格模式只允许 pnpm 为本轮精确 cohort 添加例外，不使用通配放宽。当前不支持 profile `.npmrc`、带凭据的 registry 配置或会加载额外代码的 pnpm 配置依赖；这些情况会在 profile pnpm 查询前或元数据准备阶段拒绝，原 profile 保留。声明在 `/tmp` 或 `/run` 中的外部 Web 工作区仅在 Host 启动预检时以只读目录描述符挂入；既有 Host peer 外链在包物化时暂时移开，结束后恢复，避免生成新的锁文件外可执行链接。
...
事务也不会修改权限、模型或 Agent 工具配置，并会拒绝不安全 owner/权限、home 内的外部状态链接、外部硬链接、挂载点、特殊文件及无法证明归属的旧事务残留；失败证据保留在同级 transaction 目录供人工排查。`--confirm-dsh-home-stopped` 对 service-aware lifecycle 表示除安装器将自行停止的 systemd units 外，其它使用整个 `DSH_HOME` 的外部/手工进程均已停止；它不是允许安装器 kill 未知进程的授权。
...
远程 npm 安装器（`install-npm.sh`）可从 `main` 直接拉取执行，但它只是薄引导器；当脚本不在 `common.sh` 旁边运行时，它从固定 `vX.Y.Z` 发布标签拉取并校验 `common.sh`。仅当参数中的最终 `--operation` 为 `upgrade` 或 `uninstall` 时，它才从同一个 tag 拉取 `lifecycle-config.mjs` 和 `lifecycle-profile.mjs`；所需资产先写入私有临时目录中的 `.download` 文件，全部通过各自内嵌的 lowercase SHA-256 校验后才原子改名并设为只读，随后才 source `common.sh`。远程 bootstrap 要求 `TMPDIR` 是绝对路径；逻辑路径上的每个符号链接组件都必须由 root 拥有并解析到目录，因此接受 macOS `/var` → `/private/var`（每用户 `TMPDIR` 所在）与 `/tmp` → `/private/tmp` 这类操作系统别名，任何非 root 拥有的符号链接组件都会在下载前被拒绝。完全解析到 canonical 路径后，最终目录必须由当前用户拥有，其非系统临时目录祖先不能由非 root/当前用户拥有或向 group/other 开放写权限；系统 `/tmp` 必须解析到 root-owned `01777` 目录。实际安装逻辑和生命周期执行器都不从 mutable `main` 执行。可分别用 `DSH_ENHANCED_INSTALL_COMMON_SHA256`、`DSH_ENHANCED_INSTALL_LIFECYCLE_CONFIG_SHA256`、`DSH_ENHANCED_INSTALL_LIFECYCLE_PROFILE_SHA256` 覆盖 digest，并用 `DSH_ENHANCED_INSTALL_REF` 覆盖固定发布 tag；所有 digest 必须是精确的 64 位小写十六进制值。`v0.1.30` 的发布准备为三个实际资产写入独立摘要，支持上述远程生命周期入口。旧 `v0.1.24` 不含两个 lifecycle helper，其全零 sentinel 会使远程 upgrade/uninstall 在下载或 source 前拒绝。本地 sibling 模式直接使用 checkout helper；checkout 中尚未发布的安装逻辑不会被远程引导器加载。
