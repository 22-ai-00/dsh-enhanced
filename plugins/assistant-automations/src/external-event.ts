...
export interface ExternalEventEnvelope {
  ...
  source: Readonly<{ id: string; kind: 'file' | 'http-json' | 'webhook'; version: string; configDigest: string }>
...
export function parseExternalEventEnvelope(value: unknown): Readonly<ExternalEventEnvelope> {
  ...
  if (kind !== 'file' && kind !== 'http-json' && kind !== 'webhook') fail('source.kind is unsupported')
  ...
  const expected = kind === 'file'
    ...
    : kind === 'http-json' ? ['https-observation', 'observed'] : ['hmac-sha256', 'source-signed']
