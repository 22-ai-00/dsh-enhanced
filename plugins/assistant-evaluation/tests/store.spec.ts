...
describe('evaluation database', () => {
  ...
  test('conservatively backfills version-three rows into a durable task projection', () => {
    ...
    expect(store.health()).toMatchObject({ schemaVersion: evaluationSchemaVersion, taskProjections: 1 })
    store.close()
  })
  ...
  test('preserves a schema-nine projection and its evidence while rebuilding the v10 subject constraint', () => {
    ...
    const migrated = openEvaluationDatabase(path)
    expect((migrated.prepare('PRAGMA user_version').get() as { user_version: number }).user_version).toBe(evaluationSchemaVersion)
  ...
  test('preserves a schema-ten projection while widening the goal-outcome subject constraint', () => {
    const path = join(root(), 'v10.sqlite')
    const store = new EvaluationStore({ path, now: () => 2_000 })
    const stored = store.append(envelope({
      situation: 'goal:goal-v10:definition:1',
      evidence: [{ kind: 'goal-step', ref: 'goal-run-v10', digest: 'sha256:v10' }],
      idempotencyKey: 'v10-goal-step-evidence',
    }))
    store.close()
    ...
    legacy.exec(`
      ...
      CREATE INDEX evaluation_task_projections_scope_time
        ON evaluation_task_projections(scope_key, updated_at DESC, subject_key);
      ${v10View.sql};
      INSERT INTO evaluation_task_projections SELECT * FROM evaluation_task_projections_v10_fixture;
      DROP TABLE evaluation_task_projections_v10_fixture;
    ...
    expect(migrated.prepare('PRAGMA foreign_key_check').all()).toEqual([])
    expect(migrated.prepare(`SELECT subject_kind, subject_ref, primary_outcome_id FROM evaluation_task_projections`).all())
      .toEqual([{ subject_kind: 'goal-step', subject_ref: 'goal-run-v10', primary_outcome_id: stored.id }])
    migrated.close()
  })
})
