...
function privateRoot(path: string): void {
  ...
  if (!stat.isDirectory() || stat.isSymbolicLink() || realpathSync(path) !== path || (stat.mode & 0o077) !== 0 || stat.uid !== process.getuid?.()) fail('private owned state root required')
}
...
function binding(scope: GoalScope, baseline: SkillDefinition, candidate: SkillDefinition, execution: HoldoutExecutionConfig, inputs: Readonly<Record<string, unknown>>, files: readonly { path: string; content: string }[]) {
  ...
  return { scopeDigest: digest(scope), baselineDigest: digest(baseline), candidateDigest: digest(candidate),
    ...
    expiresAt: execution.expiresAt, repeats: execution.repeats, limits }
}
function validBegin(value: unknown, expected: ReturnType<typeof binding>, pinnedPublicKey: string): value is BeginResult {
  ...
  if (typeof data.sessionId !== 'string' || data.sessionId.length === 0 || !hex(data.planDigest) || !hex(data.datasetDigest) || data.publicKey !== pinnedPublicKey || data.scopeDigest !== expected.scopeDigest
    || data.baselineDigest !== expected.baselineDigest || data.candidateDigest !== expected.candidateDigest || data.budgetDigest !== expected.budgetDigest || data.expiresAt !== expected.expiresAt || data.repeats !== expected.repeats
...
export async function qualifyHoldout(input: HoldoutQualificationInput): Promise<HoldoutQualificationResult> {
  ...
  if (inside(scope.workspace, execution.stateRoot) || inside(execution.stateRoot, scope.workspace)) fail('state root overlaps qualification workspace'); privateRoot(execution.stateRoot)
  ...
  const frozen = binding(scope, arms.baseline, arms.candidate, execution, inputs, files)
  const revalidate = () => { input.signal.throwIfAborted(); if (Date.now() >= execution.expiresAt) fail('qualification expired'); input.authorize() }
  revalidate()
