...
export function validateRepairProfiles(values: unknown, holdouts: readonly ExternalHoldoutProfile[]): readonly RepairContinuationProfile[] {
  ...
  const allowed = ['id', 'scope', 'skillName', 'taskFamilyId', 'description', 'bindings', 'externalHoldoutProfileId', 'provider', 'model', 'allowedTools', 'maxGoalRounds', 'maxModelCalls', 'maxToolCalls', 'maxOutputTokens', 'maxDurationMs', 'canaryRuns', 'maxCanaryRuns']
  const profiles = values.map((value: unknown) => {
    if (!plain(value) || Object.keys(value).some(key => !allowed.includes(key)) || !plain(value.scope)
      ...
      || !integer(value.canaryRuns, 1, 32) || !integer(value.maxCanaryRuns, Number(value.canaryRuns), 100)) throw new Error('assistant-skills: invalid repair profile')
    ...
    return structuredClone(value) as unknown as RepairContinuationProfile
  })
