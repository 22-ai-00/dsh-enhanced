...
export class AcceptanceStore {
  ...
  getTaskContract(identity: { scope: { workspace: string; preset: string }; owner: { principalRecordId: string; principalVersion: number }; task: AcceptanceTaskIdentity }): TaskAcceptanceContract | null {
    ...
    safeId(identity.owner?.principalRecordId, 'owner principalRecordId'); if (!Number.isSafeInteger(identity.owner.principalVersion) || identity.owner.principalVersion < 1) fail('invalid-input', 'owner principalVersion is invalid'); if (identity.task.kind !== 'automation-run' && identity.task.kind !== 'foreground-turn' && identity.task.kind !== 'goal-step' && identity.task.kind !== 'goal-outcome') fail('invalid-input', 'task kind is invalid'); safeId(identity.task.ref, 'task ref')
