...
async function nativeGoalPlugins(): Promise<Readonly<{
  ...
  goalRoundDriver: { inject: readonly string[], apply: (ctx: Context) => void }
}>> {
  ...
  const driverEntry = goalsRequire.resolve('@deepseek-ai/dsh-goal-round-driver')
  const [goal, tools, driver] = await Promise.all([
    import(pathToFileURL(goalEntry).href),
    import(pathToFileURL(toolEntry).href),
    import(pathToFileURL(driverEntry).href),
  ]) as [
    { default: new (ctx: Context) => unknown },
    { inject: readonly string[], apply: (ctx: Context, config: object) => void },
    { inject: readonly string[], apply: (ctx: Context) => void },
  ]
...
async function runtimeHarness(
  ...
  await ctx.plugin(AssistantDeliveryService, { databasePath: join(root, 'delivery.sqlite'), spoolPath: join(root, 'spool'),
    ...
    ...(permissions?.goalContinuationTimeoutMs === undefined
      ? {}
      : { agentGoalContinuationTimeoutMs: permissions.goalContinuationTimeoutMs }) })
...
describe('real rc.1 delivery Agent runtime', () => {
  ...
  test('indexes native business goals for the live owner and restores an awaiting-verification checkpoint', async () => {
    ...
    const goalPolicy = {
      ...
      actions: ['create', 'observe', 'inspect', 'focus', 'checkpoint', 'snapshot', 'edit', 'pause', 'resume', 'clear'],
  ...
  test('controls a live owner goal through edit pause resume stale revision and clear', async () => {
    const root = await mkdtemp(join(tmpdir(), 'assistant-delivery-native-goal-control-'))
    roots.push(root)
    const ownerId = 'lark/bot-1/tenant-a/ou_owner'
    const goalPolicy = {
      id: 'owner-business-goal-control', effect: 'allow' as const,
    ...
    const toolPolicy = (name: string) => ({
      id: `owner-${name}`, effect: 'allow' as const,
      subject: { kind: 'agent' as const, id: 'primary', workspace: root, principal: ownerId },
    ...
    const fixture = await runtimeHarness(root, new Map(), undefined, undefined, root, undefined, 'primary', true, 'probe', undefined, {
      policyRules: [goalPolicy, toolPolicy('goal_create'), toolPolicy('goal_checkpoint'), toolPolicy('goal_control')],
    ...
    let controlled: ReturnType<AssistantGoalsService['list']>[number] | undefined
    fixture.ctx.on('agent/pre-step', async ({ agent, signal }, next) => {
      if (fixture.service.currentPreferenceTurn(agent) === undefined || controlled !== undefined) return await next()
      const create = await fixture.ctx.tools.execute({
        callId: ToolCallId('owner-goal-control-create'), name: 'goal_create', agent, signal,
        arguments: { objective: 'Preserve the original objective', max_goal_rounds: 2 },
      })
      if (create.isError) throw new Error(`goal_control setup create failed: ${JSON.stringify(create.content)}`)
      const initial = fixture.ctx.assistantGoals.list(agent).at(0)
      if (initial === undefined) throw new Error('goal_control setup goal missing')
      const checkpoint = await fixture.ctx.tools.execute({
        callId: ToolCallId('owner-goal-control-checkpoint'), name: 'goal_checkpoint', agent, signal,
        arguments: { goal_id: initial.id, expected_version: initial.version, next_step: 'Retain this checkpoint', blockers: [], assumptions: [], evidence_refs: ['delivery:owner'], dependencies: [] },
      })
      if (checkpoint.isError) throw new Error('goal_control setup checkpoint failed')
      const edit = await fixture.ctx.tools.execute({
        callId: ToolCallId('owner-goal-control-edit'), name: 'goal_control', agent, signal,
        arguments: { goal_id: initial.id, expected_revision: 1, operation: 'edit', objective: 'Revised native objective' },
      })
      if (edit.isError) throw new Error(`goal_control edit failed: ${JSON.stringify(edit.content)}`)
      const stalePause = await fixture.ctx.tools.execute({
        callId: ToolCallId('owner-goal-control-stale'), name: 'goal_control', agent, signal,
        arguments: { goal_id: initial.id, expected_revision: 1, operation: 'pause' },
      })
      if (!stalePause.isError) throw new Error('goal_control accepted a stale native revision')
      for (const [callId, expectedRevision, operation] of [
        ['owner-goal-control-pause', 2, 'pause'], ['owner-goal-control-resume', 3, 'resume'], ['owner-goal-control-clear', 4, 'clear'],
      ] as const) {
        const result = await fixture.ctx.tools.execute({ callId: ToolCallId(callId), name: 'goal_control', agent, signal,
          arguments: { goal_id: initial.id, expected_revision: expectedRevision, operation } })
        if (result.isError) throw new Error(`goal_control ${operation} failed: ${JSON.stringify(result.content)}`)
      }
      controlled = fixture.ctx.assistantGoals.list(agent).at(0)
      return await next()
    })
    const pairing = fixture.service.issuePairing('test', principal)
    fixture.service.confirmPairing({ challengeId: pairing.challenge.id, principal, code: pairing.code })
    await fixture.service.acceptInbound(message('evt-native-goal-control', 'Control my durable delivery goal'))
    await drive(fixture.service)
    expect(controlled).toMatchObject({
      originalObjective: 'Preserve the original objective', native: { objective: 'Revised native objective', phase: 'cleared', revision: 5 },
      checkpoint: { nextStep: 'Retain this checkpoint', evidenceRefs: ['delivery:owner'] },
    })
    await fixture.ctx.fiber.restart()
  })

  test('keeps a paused owner goal disarmed across restore until goal_control resume drives native rounds', async () => {
    const root = await mkdtemp(join(tmpdir(), 'assistant-delivery-native-goal-round-driver-'))
    roots.push(root)
    const saved = new Map<string, SavedSession>()
    const ownerId = 'lark/bot-1/tenant-a/ou_owner'
    const goalPolicy = {
      id: 'owner-business-goal-round-driver', effect: 'allow' as const,
      subject: { kind: 'agent' as const, id: 'primary', workspace: root, principal: ownerId },
      actions: ['create', 'observe', 'inspect', 'focus', 'checkpoint', 'snapshot', 'pause', 'resume'],
      resource: { kind: 'goal' as const, id: 'business-context' }, context: { initiators: ['external' as const] },
    }
    const toolPolicy = (name: string) => ({
      id: `owner-round-driver-${name}`, effect: 'allow' as const,
      subject: { kind: 'agent' as const, id: 'primary', workspace: root, principal: ownerId },
      actions: ['execute'], resource: { kind: 'tool' as const, id: name }, context: { initiators: ['external' as const] },
    })
    const mountGoals = async (ctx: Context) => {
    ...
    const first = await runtimeHarness(root, saved, undefined, undefined, root, undefined, 'primary', true, 'probe', undefined, options)
    await mountGoals(first.ctx)
    let paused: ReturnType<AssistantGoalsService['list']>[number] | undefined
    first.ctx.on('agent/pre-step', async ({ agent, signal }, next) => {
      if (first.service.currentPreferenceTurn(agent) === undefined || paused !== undefined) return await next()
      const created = await first.ctx.tools.execute({ callId: ToolCallId('round-driver-create'), name: 'goal_create', agent, signal,
        arguments: { objective: 'Drive exactly two retained goal rounds', max_goal_rounds: 2 } })
      if (created.isError) throw new Error(`round-driver create failed: ${JSON.stringify(created.content)}`)
      const record = first.ctx.assistantGoals.list(agent).at(0)
      if (record === undefined) throw new Error('round-driver goal was not indexed')
      const checkpoint = await first.ctx.tools.execute({ callId: ToolCallId('round-driver-checkpoint'), name: 'goal_checkpoint', agent, signal,
        arguments: { goal_id: record.id, expected_version: record.version, next_step: 'Resume only when the owner requests it', blockers: [], assumptions: [], evidence_refs: ['delivery:round-driver'], dependencies: [] } })
      if (checkpoint.isError) throw new Error('round-driver checkpoint failed')
      const pause = await first.ctx.tools.execute({ callId: ToolCallId('round-driver-pause'), name: 'goal_control', agent, signal,
        arguments: { goal_id: record.id, expected_revision: record.native.revision, operation: 'pause' } })
      if (pause.isError) throw new Error(`round-driver pause failed: ${JSON.stringify(pause.content)}`)
      paused = first.ctx.assistantGoals.list(agent).at(0)
      return await next()
    })
    const pairing = first.service.issuePairing('test', principal)
    first.service.confirmPairing({ challengeId: pairing.challenge.id, principal, code: pairing.code })
    await first.service.acceptInbound(message('evt-native-goal-round-driver-create', 'Pause a durable goal before automatic work'))
    await drive(first.service)
    expect(paused).toMatchObject({ native: { phase: 'paused' }, checkpoint: { nextStep: 'Resume only when the owner requests it' } })
    const goalId = paused!.id
    const nativeGoalId = paused!.native.goalId
    await first.ctx.fiber.restart()

    const reopened = await runtimeHarness(root, saved, undefined, undefined, root, undefined, 'primary', true, 'probe', undefined, options)
    await mountGoals(reopened.ctx)
    let resumedSnapshot = ''
    ...
    reopened.ctx.on('agent/pre-step', async ({ agent, signal }, next) => {
      if (reopened.service.currentPreferenceTurn(agent) === undefined) {
        ...
        if (live !== undefined && live.roundsStarted > 0 && autonomousControl === undefined) {
          autonomousControl = await reopened.ctx.tools.execute({
            callId: ToolCallId('round-driver-autonomous-control'), name: 'goal_control', agent, signal,
            arguments: { goal_id: goalId, expected_revision: live.id === nativeGoalId ? 3 : 1, operation: 'pause' },
          })
        }
        return await next()
      }
      if (resumedRecord !== undefined) return await next()
      const existing = reopened.ctx.assistantGoals.list(agent).at(0)
      if (existing === undefined || existing.id !== goalId || existing.native.phase !== 'paused') throw new Error('paused goal was not restored as owner context')
      const resume = await reopened.ctx.tools.execute({ callId: ToolCallId('round-driver-resume'), name: 'goal_control', agent, signal,
        arguments: { goal_id: goalId, expected_revision: existing.native.revision, operation: 'resume' } })
    ...
    await drive(reopened.service)
    const sessionId = runtimeStore(reopened.service).getActiveBinding(conversation)?.sessionId
    const rounds = sessionId === undefined ? [] : saved.get(sessionId)?.events.flatMap(event => {
      const candidate = event as unknown as { type: string, data?: { source?: { kind?: string, round?: number } } }
      return candidate.type === 'user/message' && candidate.data?.source?.kind === 'goal' ? [candidate.data.source] : []
    }) ?? []
    expect(resumedSnapshot).toContain('Resume only when the owner requests it')
    expect(resumedRecord).toMatchObject({ id: goalId, native: { goalId: nativeGoalId } })
  ...
  test.each([
    ['Confirmed result', 'achieved'],
    ['Missing required evidence', 'not-achieved'],
  ] as const)('freezes a v2 goal-step acceptance contract before the native round and verifies its artifact (%s)', async (report, expected) => {
    const root = await mkdtemp(join(tmpdir(), 'assistant-delivery-goal-step-v2-'))
    roots.push(root)
    const ownerId = 'lark/bot-1/tenant-a/ou_owner'
    const rule = (name: string) => ({ id: `goal-step-${name}`, effect: 'allow' as const,
      subject: { kind: 'agent' as const, id: 'primary', workspace: root, principal: ownerId }, actions: ['execute'],
      resource: { kind: 'tool' as const, id: name }, context: { initiators: ['external' as const] } })
    const goalRule = { id: 'goal-step-goal', effect: 'allow' as const,
      ...
      resource: { kind: 'goal' as const, id: 'business-context' }, context: { initiators: ['external' as const] } }
    const fixture = await runtimeHarness(root, new Map(), undefined, undefined, root, undefined, 'primary', true, 'probe', undefined, {
      policyRules: [goalRule, rule('goal_create')], presets: canonicalPermissionPresets, seedDefaultPreset: 'danger-full-access', provideApproval: false,
      goalContinuationTimeoutMs: 5_000,
    })
    const native = await nativeGoalPlugins()
    await fixture.ctx.plugin(native.GoalService as never, {} as never)
    await fixture.ctx.plugin(native.goalTools as never, {} as never)
    await fixture.ctx.plugin(native.goalRoundDriver as never, {} as never)
    await fixture.ctx.plugin(AssistantGoalsService, { databasePath: join(root, 'goals.sqlite'), verifyNativeRounds: true, stepMaxDurationMs: 5_000 } as never)
    const pairing = fixture.service.issuePairing('test', principal)
    fixture.service.confirmPairing({ challengeId: pairing.challenge.id, principal, code: pairing.code })
    const owner = runtimeStore(fixture.service).getPrincipal(principal)!
    const objective = 'Verify the native goal-step report'
    await writeFile(join(root, 'report.md'), report)
    ...
    await fixture.ctx.plugin(AssistantVerifierService, { databasePath: join(root, 'verification.sqlite'), tickIntervalMs: 0, requireAcceptance: false,
      authorities: [authority], profiles: [{ id: 'goal-step', version: 2, scope: { workspace: root, preset: 'primary' },
    ...
    fixture.ctx.on('agent/pre-step', async ({ agent: current, signal }, next) => {
      if (fixture.service.currentPreferenceTurn(current) === undefined || nativeGoals(fixture.ctx).get(current) !== undefined) return await next()
      const created = await fixture.ctx.tools.execute({ callId: ToolCallId('goal-step-v2-create'), name: 'goal_create', agent: current, signal, arguments: { objective, max_goal_rounds: 1 } })
      if (created.isError) throw new Error('goal-step setup create rejected')
      return await next()
    })
    let goalContractId: string | undefined
    const originalStream = fixture.llm.stream.bind(fixture.llm)
    vi.spyOn(fixture.llm, 'stream').mockImplementation(async function* (options) {
      // The first request is the foreground owner turn.  The second is the
      // native goal round, and its contract must already be durable before
      // the adapter can observe a paid model request.
      if (fixture.llm.requests.length >= 1) {
        const database = new DatabaseSync(join(root, 'verification.sqlite'), { readOnly: true })
        try {
          const row = database.prepare('SELECT payload FROM acceptance_contracts WHERE task_kind = ?').get('goal-step') as { payload: string } | undefined
          expect(row).toBeDefined()
          const contract = JSON.parse(row!.payload) as { id: string }
          goalContractId = contract.id
          expect(contract).toMatchObject({ protocol: 'task-acceptance/v2', objective,
            task: { kind: 'goal-step', goal: { definitionVersion: 1, sessionId: expect.any(String), nativeGoalId: expect.any(String), runId: expect.any(String) } } })
        } finally { database.close() }
      }
      yield* originalStream(options)
    })
    await fixture.service.acceptInbound(message(`evt-goal-step-v2-${expected}`, objective))
    await drive(fixture.service)
    await (fixture.ctx.assistantGoals as unknown as { whenIdle(): Promise<void> }).whenIdle()
    expect(goalContractId).toBeDefined()
    await fixture.ctx.assistantVerifier.tick()
    expect(fixture.ctx.assistantVerifier.inspect(goalContractId!)).toMatchObject({ state: 'done', receipt: { objectiveStatus: expected } })
    await fixture.ctx.fiber.restart()
  })

  test('feeds an independently failed first native goal-step into its second automatically driven round', async () => {
    const root = await mkdtemp(join(tmpdir(), 'assistant-delivery-goal-step-feedback-rounds-'))
    roots.push(root)
    const ownerId = 'lark/bot-1/tenant-a/ou_owner'
    const objective = 'Repair the required native goal-step report'
    const rule = (name: string) => ({ id: `goal-step-feedback-${name}`, effect: 'allow' as const,
      subject: { kind: 'agent' as const, id: 'primary', workspace: root, principal: ownerId }, actions: ['execute'],
      resource: { kind: 'tool' as const, id: name }, context: { initiators: ['external' as const] } })
    const goalRule = { id: 'goal-step-feedback-goal', effect: 'allow' as const,
      subject: { kind: 'agent' as const, id: 'primary', workspace: root, principal: ownerId }, actions: ['create', 'observe', 'inspect', 'focus', 'checkpoint', 'snapshot', 'execute'],
      resource: { kind: 'goal' as const, id: 'business-context' }, context: { initiators: ['external' as const] } }
    const fixture = await runtimeHarness(root, new Map(), undefined, undefined, root, undefined, 'primary', true, 'probe', undefined, {
    ...
    await fixture.ctx.plugin(AssistantGoalsService, {
      databasePath: join(root, 'goals.sqlite'), verifyNativeRounds: true, stepMaxDurationMs: 5_000,
    } as never)
    const pairing = fixture.service.issuePairing('test', principal)
    fixture.service.confirmPairing({ challengeId: pairing.challenge.id, principal, code: pairing.code })
    const owner = runtimeStore(fixture.service).getPrincipal(principal)!
    ...
    await fixture.ctx.plugin(AssistantVerifierService, {
      ...
      authorities: [authority], profiles: [{ id: 'goal-step-feedback', version: 1,
        scope: { workspace: root, preset: 'primary' },
    ...
    fixture.ctx.on('agent/pre-step', async ({ agent, signal }, next) => {
      if (fixture.service.currentPreferenceTurn(agent) !== undefined && nativeGoals(fixture.ctx).get(agent) === undefined) {
        const created = await fixture.ctx.tools.execute({ callId: ToolCallId('goal-step-feedback-create'), name: 'goal_create', agent, signal,
          arguments: { objective, max_goal_rounds: 2 } })
        if (created.isError) throw new Error(`goal-step feedback setup rejected: ${JSON.stringify(created.content)}`)
      }
      return await next()
    })
    ...
    vi.spyOn(fixture.llm, 'stream').mockImplementation(async function* (options) {
      ...
      if (goalRequests.length === 1) {
        expect(prompt).not.toContain('not-achieved')
        yield { type: 'block-start', index: 0, blockType: 'text' }
        yield { type: 'text-delta', index: 0, text: 'Initial native attempt.' }
        yield { type: 'block-end', index: 0, block: { type: 'text', text: 'Initial native attempt.' } }
        yield { type: 'finish', reason: { kind: 'stop' } }
        return
      }
      expect(goalRequests).toHaveLength(2)
      expect(prompt).toContain('not-achieved')
      expect(prompt).toContain('"id":"result"')
      expect(prompt).toContain('"status":"failed"')
      expect(prompt).toContain('"reason":"required-text-missing"')
      expect(prompt).toContain('"goalOutcome":"unverified"')
      expect(prompt).toContain('"nextAction":"revise-plan"')
      await writeFile(join(root, 'report.md'), 'Confirmed result')
      yield { type: 'block-start', index: 0, blockType: 'text' }
      yield { type: 'text-delta', index: 0, text: 'Confirmed result' }
      yield { type: 'block-end', index: 0, block: { type: 'text', text: 'Confirmed result' } }
      yield { type: 'finish', reason: { kind: 'stop' } }
    })

    await fixture.service.acceptInbound(message('evt-goal-step-feedback-rounds', objective))
    await drive(fixture.service)
    await (fixture.ctx.assistantGoals as unknown as { whenIdle(): Promise<void> }).whenIdle()
    ...
    try {
      contracts = (database.prepare("SELECT id FROM acceptance_contracts WHERE task_kind = 'goal-step' ORDER BY rowid ASC").all() as Array<{ id: string }>)
      expect(contracts).toHaveLength(2)
    } finally { database.close() }
    expect(fixture.ctx.assistantVerifier.inspect(contracts[0]!.id)).toMatchObject({
      state: 'done', execution: { status: 'succeeded', quiescent: true }, receipt: { objectiveStatus: 'not-achieved' },
    })
    expect(fixture.ctx.assistantVerifier.inspect(contracts[1]!.id)).toMatchObject({
    ...
    try {
      ...
      expect(native.phase).not.toBe('complete')
    } finally { goals.close() }
    await fixture.ctx.fiber.restart()
  })
  ...
  test('rejects a native goal round before its model request when no goal-step profile exists', async () => {
    ...
    const ownerId = 'lark/bot-1/tenant-a/ou_owner'
    const fixture = await runtimeHarness(root, new Map(), undefined, undefined, root, undefined, 'primary', true, 'probe', undefined, {
      policyRules: [{ id: 'goal-step-no-profile-goal', effect: 'allow' as const, subject: { kind: 'agent' as const, id: 'primary', workspace: root, principal: ownerId }, actions: ['create', 'observe', 'inspect', 'focus', 'checkpoint', 'snapshot', 'execute'], resource: { kind: 'goal' as const, id: 'business-context' }, context: { initiators: ['external' as const] } },
        { id: 'goal-step-no-profile-tool', effect: 'allow' as const, subject: { kind: 'agent' as const, id: 'primary', workspace: root, principal: ownerId }, actions: ['execute'], resource: { kind: 'tool' as const, id: 'goal_create' }, context: { initiators: ['external' as const] } }],
      presets: canonicalPermissionPresets, seedDefaultPreset: 'danger-full-access', provideApproval: false, goalContinuationTimeoutMs: 5_000,
    })
    const native = await nativeGoalPlugins()
    await fixture.ctx.plugin(native.GoalService as never, {} as never); await fixture.ctx.plugin(native.goalTools as never, {} as never); await fixture.ctx.plugin(native.goalRoundDriver as never, {} as never)
    ...
    fixture.ctx.on('agent/pre-step', async ({ agent, signal }, next) => {
      if (fixture.service.currentPreferenceTurn(agent) !== undefined && nativeGoals(fixture.ctx).get(agent) === undefined) {
        const created = await fixture.ctx.tools.execute({ callId: ToolCallId('goal-step-no-profile-create'), name: 'goal_create', agent, signal, arguments: { objective: 'No profile may run', max_goal_rounds: 1 } })
        if (created.isError) throw new Error('goal-step no-profile create rejected')
      }
      return await next()
    })
    const pairing = fixture.service.issuePairing('test', principal); fixture.service.confirmPairing({ challengeId: pairing.challenge.id, principal, code: pairing.code })
    await fixture.service.acceptInbound(message('evt-goal-step-no-profile', 'No profile may run'))
    await drive(fixture.service)
    expect(fixture.llm.requests).toHaveLength(1)
    await fixture.ctx.fiber.restart()
  })

  test.each(['objective', 'round-limit', 'complete', 'timeout'] as const)('turns a native goal step unknown when %s changes after a goal tool call', async change => {
    const root = await mkdtemp(join(tmpdir(), 'assistant-delivery-goal-step-definition-change-'))
    roots.push(root)
    const ownerId = 'lark/bot-1/tenant-a/ou_owner'; const objective = 'Keep this definition frozen'
    const policy = (id: string, actions: string[], resource: { kind: 'goal' | 'tool'; id: string }) => ({ id, effect: 'allow' as const, subject: { kind: 'agent' as const, id: 'primary', workspace: root, principal: ownerId }, actions, resource, context: { initiators: ['external' as const] } })
    const fixture = await runtimeHarness(root, new Map(), undefined, undefined, root, undefined, 'primary', true, 'probe', undefined, {
      policyRules: [policy('definition-goal', ['create', 'observe', 'inspect', 'focus', 'checkpoint', 'snapshot', 'execute'], { kind: 'goal', id: 'business-context' }), policy('definition-create', ['execute'], { kind: 'tool', id: 'goal_create' }), policy('definition-probe', ['execute'], { kind: 'tool', id: 'preset_probe' })],
      presets: canonicalPermissionPresets, seedDefaultPreset: 'danger-full-access', provideApproval: false, allowPresetProbeExecution: true, goalContinuationTimeoutMs: 5_000,
    })
    const native = await nativeGoalPlugins(); await fixture.ctx.plugin(native.GoalService as never, {} as never); await fixture.ctx.plugin(native.goalTools as never, {} as never); await fixture.ctx.plugin(native.goalRoundDriver as never, {} as never)
    ...
    fixture.presetExecute.mockImplementationOnce(async () => {
      ...
      if (!goal) throw new Error('missing native goal')
      if (change === 'timeout') {
        if (goalSignal === undefined) throw new Error('missing native goal signal')
        await new Promise<void>(resolve => {
          if (goalSignal!.aborted) resolve()
          else goalSignal!.addEventListener('abort', () => resolve(), { once: true })
        })
        // The running tool ignores cancellation until this point. Even a new
        // signal cannot grant a second tool call after the admission expired.
        const late = await goalAgent!.ctx.tools.execute({
          callId: ToolCallId('expired-goal-step-late-tool'), name: 'preset_probe',
          agent: goalAgent!, signal: new AbortController().signal, arguments: {},
        })
        expect(late.isError).toBe(true)
      } else if (change === 'complete') nativeGoals(fixture.ctx).complete(goalAgent!, { id: goal.id, revision: goal.revision })
    ...
    const ledger = new DatabaseSync(join(root, 'verification.sqlite'), { readOnly: true })
    const acceptedGoal = JSON.parse((ledger.prepare("SELECT payload FROM acceptance_contracts WHERE task_kind = 'goal-step'").get() as { payload: string }).payload) as { id: string; objective: string }
    ledger.close()
    expect(acceptedGoal.objective).toBe(objective)
    const goals = new DatabaseSync(join(root, 'goals.sqlite'), { readOnly: true })
    const definition = JSON.parse((goals.prepare('SELECT definition_json FROM goal_records').get() as { definition_json: string }).definition_json)
    goals.close()
    expect(definition).toMatchObject(change === 'objective' ? { version: 2, objective: 'Changed after dispatch' } : { version: 1, objective })
    await fixture.ctx.assistantVerifier.tick()
    expect(fixture.ctx.assistantVerifier.inspect(acceptedGoal.id)).toMatchObject({ execution: { status: 'unknown', quiescent: false }, receipt: { objectiveStatus: 'unknown' } })
    await fixture.ctx.fiber.restart()
  })
  ...
  test('does not expose or adopt an old owner goal after a real owner handoff', async () => {
    ...
    const controlToolRule = (id: string) => ({
      id: `${id}-control`, effect: 'allow' as const,
      subject: { kind: 'agent' as const, id: 'primary', workspace: root, principal: id },
      actions: ['execute'], resource: { kind: 'tool' as const, id: 'goal_control' }, context: { initiators: ['external' as const] },
    })
    const replacementRules = [
      ...
      goalRule(externalId(replacement)), toolRule(externalId(replacement)), contextToolRule(externalId(replacement)), createToolRule(externalId(replacement)), controlToolRule(externalId(replacement)),
    ]
    const fixture = await runtimeHarness(root, saved, undefined, undefined, root, undefined, 'primary', true, 'probe', undefined, {
      policyRules: [goalRule(externalId(principal)), toolRule(externalId(principal)), contextToolRule(externalId(principal)), createToolRule(externalId(principal)), controlToolRule(externalId(principal)), ...replacementRules],
      presets: canonicalPermissionPresets, seedDefaultPreset: 'danger-full-access', provideApproval: false,
    })
    const native = await nativeGoalPlugins()
    await fixture.ctx.plugin(native.GoalService as never, {} as never)
    ...
    let replacementControl: Awaited<ReturnType<typeof fixture.ctx.tools.execute>> | undefined
    fixture.ctx.on('agent/pre-step', async ({ agent, signal }, next) => {
      ...
      if (turn?.principalId === externalId(replacement) && oldRecord !== undefined) {
        ...
        replacementControl = await fixture.ctx.tools.execute({
          callId: ToolCallId('replacement-old-goal-control'), name: 'goal_control', agent, signal,
          arguments: { goal_id: oldRecord.id, expected_revision: oldRecord.native.revision, operation: 'pause' },
        })
      }
      return await next()
    })
