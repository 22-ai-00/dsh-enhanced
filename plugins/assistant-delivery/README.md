...
| `toolApprovalPreparationTimeoutMs` | `10000` | 卡片前持久化准备上限；范围 100–30000 毫秒，且不超过即时审批总有效期；超时不出卡、不执行 |
...
这类工具卡只在 `toolApprovalTtlMs` 内和当前进程的 open turn 中有效；调用方 abort 才是 `cancelled`，超时、过期、adapter 异常/断线/注销、Host 重启或 Delivery dispose 都 fail closed 为 `unavailable`。卡片未送达时只发送一条明确说明“本次不会执行”的失败提示，不会先刷一条不存在的待处理卡片通知。旧卡不恢复、不自动重放，也不进入 Outbox。它不同于上面的 durable Policy proposal：proposal 有持久 operation、CAS settlement 和受控 recovery，适合延迟审批；即时工具授权只允许同一 live call 的一次结果。

当当前前台 Inbox 与 owner 回合可独立重验时，Delivery 会经正常授权和 durable Outbox 在**请求来源会话**发送无参数的提示：私聊提示处理授权卡，群聊/话题明确提示去已配对私聊授权。DM 缺失/不唯一、卡片配置缺失或准备超时时，提示如何恢复，不会把飞书请求转进无浏览器的 Web 等待。提示只是消息，不是授权，也不能绕过发送策略。卡片本身仍只属于当前 live 调用，不持久重放。

进度消费原生 `approval/decided`，分别展示批准、拒绝、取消、不可用/超时；没有人工点击的超时不再误报为用户拒绝，也不会永久保留“正在确认权限”。
