...
- 执行进度使用飞书原生 `/open-apis/im/v1/message_cot` 展示：`showProgress: false` 可关闭。该接口不在固定版 Node SDK 的高层 API 中，因此作为可降级展示使用；创建或更新失败只记录 channel health，不会重跑任务或影响最终 Outbox 回复。事件内层 JSON 必须使用 API 的 `message_id` / `content` 等 snake_case 字段；不能把内部 TypeScript 的 `messageId` / `delta` 直接透传，否则飞书会接受事件却显示空白文本块。
