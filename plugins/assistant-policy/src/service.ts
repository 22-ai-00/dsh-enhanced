...
export class AssistantPolicyService extends Service {
  ...
  registerPreauthorizedTool(
    ...
    const expectedCaller = definition.name === 'action_github_commit' ? 'dsh-enhanced-assistant-actions'
      ...
      : ['goal_create', 'goal_schedule'].includes(definition.name) ? 'dsh-enhanced-assistant-goals' : undefined
    if (expectedCaller === undefined || caller.fiber.name !== expectedCaller) {
      throw new Error('assistant-policy: preauthorization is reserved for assistant-actions action_github_commit or assistant-isolation isolation_run or assistant-goals goal_create/goal_schedule')
    }
