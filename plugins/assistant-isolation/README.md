...
Within a configured workspace/preset, the model can call `isolation_run`, `goal_context` and `goal_checkpoint`, plus `action_github_commit` only when the trusted assistant-actions broker has preauthorized that exact registered tool and request. This also blocks Host `bash`, `run_code` and subagent dispatch through ToolRuntime. Revocation or expiry does not release that restriction while the configured scope remains loaded. This is trusted Host routing around the OS-isolated worker; a trusted operator can change or remove the plugin configuration.
