...
| Surface | Authority |
| --- | --- |
| Filesystem | Host owns a `0700` state directory, `0600` ledger and per-job staging. The worker sees its byte/inode-limited tmpfs workspace plus its configured image. Console/artifact results persist in the private ledger. No writable Host bind is exposed. Audit/result retention remains pending; a workspace quota does not bound cumulative ledger growth. |
