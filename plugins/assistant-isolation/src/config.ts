...
export const defaultLimits: IsolationLimits = { maxDurationMs: 60_000, maxInputBytes: 1_048_576,
  maxOutputBytes: 65_536, maxArtifactBytes: 262_144, maxFiles: 32, memoryMiB: 256, pidsLimit: 64, cpus: 1 }
...
export const Config: Schema<Config> = Schema.object({
  ...
  limits: Schema.object({ maxDurationMs: integer(300_000), maxInputBytes: integer(1_048_576),
    ...
    memoryMiB: integer(4096), pidsLimit: integer(512), cpus: Schema.number().min(0.1).max(8) }),
...
export function validateConfig(input: Config): Required<Config> & { limits: IsolationLimits } {
  ...
  const maxima: IsolationLimits = { maxDurationMs: 300_000, maxInputBytes: 1_048_576, maxOutputBytes: 262_144,
    maxArtifactBytes: 1_048_576, maxFiles: 128, memoryMiB: 4096, pidsLimit: 512, cpus: 8 }
  ...
  return { stateRoot, dockerPath, image, grants, limits, maxConcurrentJobs }
}
