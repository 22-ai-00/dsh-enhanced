...
type Row = { id: string; intent_json: string; state: GoalEventWait['state']; reason: string | null; sequence: number | null; envelope_canonical: string | null; envelope_digest: string | null; wake_json: string | null; scope_key: string; goal_id: string; session_id: string; native_goal_id: string; native_revision: number }
...
function source(value: unknown): GoalEventSourceSnapshot {
  if (!exact(value, ['protocol', 'sourceId', 'kind', 'version', 'configDigest', 'target', 'highWaterSequence']) || value.protocol !== 'dsh-event-source/v1'
    || !text(value.sourceId) || !['file', 'http-json', 'webhook'].includes(value.kind as string) || !text(value.version, 200)
...
export function validateGoalEventWaitIntent(value: GoalEventWaitIntent): GoalEventWaitIntent {
  if (!exact(value, ['id', 'wake', 'source', 'createdAt', 'expiresAt', 'runTimeoutMs']) || !text(value.id) || !integer(value.createdAt) || !integer(value.expiresAt) || value.expiresAt <= value.createdAt || value.expiresAt - value.createdAt > 31 * 86_400_000 || !integer(value.runTimeoutMs) || value.runTimeoutMs < 1_000 || value.runTimeoutMs > 300_000) fail()
  ...
  return freeze({ id: value.id, wake: (({ id: _id, at: _at, expiresAt: _expiresAt, ...rest }) => rest)(validated), source: snapshot, createdAt: value.createdAt, expiresAt: value.expiresAt, runTimeoutMs: value.runTimeoutMs })
}
...
function wait(row: Row): GoalEventWait {
  ...
  if (row.id !== intent.id || row.scope_key !== acceptanceCanonicalJson(intent.wake.scope) || row.goal_id !== intent.wake.goalId || row.session_id !== intent.wake.native.sessionId || row.native_goal_id !== intent.wake.native.goalId || row.native_revision !== intent.wake.native.revision) fail('schema')
...
export class GoalEventWaitStore {
  ...
  #allRows(): Row[] { return this.#database.prepare('SELECT id, intent_json, state, reason, sequence, envelope_canonical, envelope_digest, wake_json, scope_key, goal_id, session_id, native_goal_id, native_revision FROM goal_event_waits').all() as Row[] }
  #get(idValue: string): GoalEventWait | undefined { const row = this.#database.prepare('SELECT id, intent_json, state, reason, sequence, envelope_canonical, envelope_digest, wake_json, scope_key, goal_id, session_id, native_goal_id, native_revision FROM goal_event_waits WHERE id = ?').get(idValue) as Row | undefined; return row === undefined ? undefined : wait(row) }
  get(idValue: string): GoalEventWait | undefined { return text(idValue) ? this.#get(idValue) : fail() }
  prepare(input: GoalEventWaitIntent): GoalEventWait { const intent = validateGoalEventWaitIntent(input); return this.#tx(() => { const old = this.#get(intent.id); if (old) { if (!same(old.intent, intent)) fail('conflict'); return old }; try { this.#database.prepare("INSERT INTO goal_event_waits(id, intent_json, state, scope_key, goal_id, session_id, native_goal_id, native_revision) VALUES (?, ?, 'waiting', ?, ?, ?, ?, ?)").run(intent.id, JSON.stringify(intent), acceptanceCanonicalJson(intent.wake.scope), intent.wake.goalId, intent.wake.native.sessionId, intent.wake.native.goalId, intent.wake.native.revision) } catch { fail('conflict') }; return this.#get(intent.id)! }) }
