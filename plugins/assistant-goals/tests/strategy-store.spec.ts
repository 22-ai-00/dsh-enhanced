...
describe('GoalStrategyStore', () => {
  ...
  it('fences competing connections while binding native children', async () => {
    ...
    try {
      ...
      expect(second.settle('strategy-a', bound.version, { children: [{ sessionId: 'child-a', stopReason: 'complete', quiescent: true }], outcome: 'advice', outputDigest: 'c'.repeat(64), quiescent: true }, 4)).toMatchObject({ state: 'settled', version: 4, outcome: 'advice' })
  ...
  it('marks incomplete work unknown on restart without reviving it', async () => {
    ...
    try {
      ...
      expect(recovered.inspect(scope, 'strategy-a')).toMatchObject({ state: 'unknown', outcome: 'unknown', completedAt: 4 })
  ...
  it('makes a non-quiescent settlement terminally unknown', () => {
    ...
    expect(store.settle('strategy-a', 3, { children: [{ sessionId: 'child-a', stopReason: 'timeout', quiescent: false }], outcome: 'execution-failed', outputDigest: 'c'.repeat(64), quiescent: false }, 4))
      .toMatchObject({ state: 'unknown', outcome: 'unknown', children: [{ sessionId: 'child-a' }] })
    store.close()
  })
  ...
  it('never binds duplicate or excess children and cannot omit a pending child at settlement', () => {
    ...
    expect(() => store.settle('strategy-a', second.version, { children: [{ sessionId: 'child-a', stopReason: 'complete', quiescent: true }], outcome: 'advice', quiescent: true }, 5)).toThrow(GoalStoreError)
    store.close()
  })
