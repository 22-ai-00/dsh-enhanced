...
owner 还可在同一对话使用 `/learning status|explain|pause|resume|rollback <exact-T1-key> confirm|forget confirm`。控制请求通过 Delivery 私有 registration 进入，不是模型工具；它绑定 exact Inbox、binding、Delivery principal row lineage 和 durable admission cursor。cursor 由 Delivery instance epoch 与严格递增的 Inbox admission sequence 组成，同一次 Inbox 重放保持不变。`status` 和 `explain` 为回执化读取，不推进 high-water；`explain` 只保存和返回 content-free T1 key/value/state/version/evidence-count 摘要。pause、resume、rollback、forget 只在 sequence 严格越过该 owner lineage 的持久 high-water 时生效，乱序控制会留下可重放的 durable no-op receipt。rollback 用一个事务 CAS 回滚该 key 的当前 active T1 并推进 projection cutoff；没有 active 值时不改偏好记录，但仍提交 cutoff 和可重放的准确 no-op receipt。pause/restart 后仍不收集、不激活、不注入；暂停期间或 forget/rollback 之前已持久排队的 projection 只会 ACK-ignore。forget 保持 pause 状态，删除较旧 control/explain/rollback 快照以防泄露已删除的值或计数。缺少 cursor 的旧 Delivery payload 会 fail-closed：事件 ACK-ignore，控制返回 stale。
