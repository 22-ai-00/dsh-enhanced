...
async function fixture() {
  ...
  const goals = { taskContext: () => ({ goal: { id: 'goal' } }), inspectWorkflowRunContext: () => ({ goalId: 'goal', goalExecutionRunId: 'run', definition: { digest: 'd'.repeat(64), version: 1 } }), inspectOwnerGoalExecution: vi.fn(() => ({ storedGoal: { definition: { digest: 'd'.repeat(64), version: 1 }, nativeAtLastObservation: { phase: 'complete' } }, outcome: { status: 'achieved' } })), inspectOwnerVerifiedArtifacts: vi.fn(() => snapshot()) }
  ...
  ctx.provide('assistantGoals' as never, goals as never); ctx.provide('assistantAutomations' as never, automations as never)
