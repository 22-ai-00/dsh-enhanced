...
`external-unix-v1` is opt-in and fail-closed. The Host config names the canonical action Unix socket, pinned broker identity/public key, client signing key, expected action-socket ownership/mode, protocol timeouts and a minimum broker generation. The separately managed daemon reads its own strict configuration and private state. That daemon configuration—not the Host mirror—is authoritative for grant revision, owner/binding generation, repository, branch, paths, limits, allowed operations, allowed inspect kinds and the credential locator. The current first stage accepts expected-head `commit` plus only `repository`, `branch`, and grant-path-scoped `file` inspection. Branch creation, pull-request creation, pull-request/check/review inspection, compensation, verified delivery and repository-goal readback are unsupported in external mode and never fall back to embedded execution.

The Host-side configuration uses the exact read-only grant projection below. `grants` must be empty in external mode. `externalGrants.allowedOperations` and `externalGrants.allowedInspectKinds` let the Host conservatively limit discovery and preauthorization before contacting the broker; external config accepts only `repository`, `branch`, and `file` inspect kinds. The projection still contains no credential locator, `clientKeyId`, client identity, policy/emergency epochs, rollback authority, or delivery authority. The broker-owned grant remains authoritative, so matching projection fields can only narrow Host exposure and cannot expand broker authority. Its `grantDigest` must match the authoritative daemon grant; `source` binds the input classification and provenance. `maxCostUnits` mirrors the grant's finite cumulative `github-api-units` budget (one unit for each currently supported commit or inspection); it is not a currency or provider-billing guarantee.
...
`action_github_commit` accepts `grantId`, `idempotencyKey`, `expectedHeadOid` (40 lowercase hex characters), `headline`, and `files: [{path, content}]`. Only configured exact paths may be added/updated, at most 32 files and 1 MiB per request including headline/path bytes. Both backends support that expected-head commit. External v1 inspection is limited to a grant-scoped repository, branch, or allowed file; file reads accept only exact-path UTF-8 blobs up to 64 KiB.

Embedded compatibility mode additionally retains pull-request, checks, and reviews inspection. Those snapshots are observed state, never proof that an earlier mutation settled. Checks and reviews return one bounded page (20 checks or 30 reviews), with `truncated` and `untrusted` metadata; an incomplete page cannot establish an overall CI/review verdict. Embedded mode can also expose `action_github_branch` and `action_github_pr` when `repoWorkflow` explicitly authorizes them. PR creation validates its receipt against both repositories and head/base refs, but GitHub provides no atomic expected-head CAS for that endpoint. External mode does not expose those reads or mutations in this first stage. Every inspection consumes the selected backend's durable `maxActions` reservation, including across restarts. Neither mode supports merge, release, force push, arbitrary endpoints, issue/review mutation or raw API access.
...
Embedded compatibility mode can add `verifiedDelivery: { ownerRouteId: <existing-owner-route>, budgetId: <finite-automation-runs-budget> }` to a grant to require accepted-artifact delivery. This facility is not available through `external-unix-v1` in the first stage; external mode rejects it rather than routing part of the workflow back into the Host. Use matching checkout versions of Goals, Verifier, Isolation and Automations. Enable the existing Automations scheduler and configure the route and budget; a goal admission with `wake` already provisions these prerequisites. The destination branch must already exist. This mode rejects the grant's direct commit, branch and PR mutation tools; bounded repository inspection remains available.
...
In embedded compatibility mode, Verifier's optional `repository-readback` authority calls the Host-only backend for the latest exact Goal's step-verified delivery. External mode does not support this API in the first stage and returns unavailable rather than reading an embedded ledger. The embedded path requires succeeded commit/PR ledger records, the original owner route and grant revision, the independently accepted source step, and the assessment's succeeded/quiescent native execution. A later assessment may verify an earlier source round; a newer pending or unknown delivery prevents fallback to an older success.

The broker reads checks, reviews, PR and branch afresh through the existing Policy, credential leases and action ledger. Each read consumes one action, with source and authorization checks across asynchronous waits. Required check names and app identities, configured reviewers, repository/base/branch and actual delivered head must match. Truncated, mismatched or unavailable data cannot pass. This interface is not a model tool, creates no events and does not complete a Goal itself. Tests use a GitHub transport fixture; real remote verification is separate.
