...
- 当前自然反馈与升级兼容改动的验证：`NODE_OPTIONS=--max-old-space-size=8192 pnpm check` 退出 0，覆盖清单校验、零警告 lint、类型检查、测试、构建与打包。根目录 670 项、36 个包 6,067 项测试通过，50 项跳过；其中 Delivery 810 项、Growth Driver 92 项、Lark 515 项通过。36 个包 dry-run pack 均成功，32 个插件包含 patch，所有包包含 README/LICENSE，未混入测试和日志。独立只读源码复核通过；最后的只读快照修复另通过 Delivery/Lark 类型检查、Delivery 构建、定向测试与 lint。
