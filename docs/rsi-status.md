...
- Evaluation 提供当前可信任务结果的持久增量读取，包括纠正和撤回；Delivery 将前台结果绑定到精确 owner 身份与原始任务，支持 `/new` 后读取旧任务并拒绝身份换代继承。它们是自动成长的来源接口；持久触发、修复调度和采用仍待接通。
...
1. 打通普通 authenticated owner 会话/任务结果 → 持久成长触发。沿 Delivery、Evaluation 与原生 Goal 读取真实使用反馈，支持重启恢复、去重和 unknown 对账；在原任务执行时持久冻结实际模型，历史任务不读取后来切换的会话模型。普通前台任务须经 owner 授权的 Host 入口创建真实原生 Goal，不能伪造现有 Skills repair 的来源 Goal；调度复用 Automations，不要求用户手动运行评测脚本。
