...
> 最新实施批次（2026-09-09，基线 `dd19370`）：WP17 已补齐 npm 远程 `upgrade` / `uninstall` 发布入口。远端引导器把 `common.sh`、`lifecycle-config.mjs`、`lifecycle-profile.mjs` 绑定到同一不可变 release tag，三份资产全部下载、独立校验 SHA-256 并封存后才执行；upgrade 在生命周期锁内先恢复旧事务、解析并预取精确 npm cohort，再以 offline/copy 模式进入真实 bubblewrap 事务，uninstall 不访问 registry。当前 `v0.1.24` 尚无两个 helper，故保持双零 sentinel，普通 install 可用而远端 upgrade/uninstall 在下载和 source 前 fail closed；下一次 `release:prepare` 才会写入三资产真实 hash，并由 tag/record 校验复核。最新定向回归为 installer **129 通过 / 1 条条件跳过（共 130 项）**、release **93 通过**；默认 `TMPDIR=/tmp` 下，记录型 npm/pnpm、真实三资产 SHA 与真实 bwrap 的高保真远端 upgrade/uninstall 和 zero sentinel 均通过，不再出现 validator `MODULE_NOT_FOUND`。隔离器把已打开的 validator fd 4 以只读 `ro-bind-data` 映射到 `/run`；生命周期同时继承并验证父目录 fd 3、per-home 锁 fd 4、全局 rendezvous 锁 fd 5，绑定完整祖先链，并在 rename/清理边界执行 staged tree syncfs 与目录 fsync。最终独立安全审计为 **PASS**。冻结快照的根 `pnpm check` 退出 **0**：352 个测试文件、4,397 项测试通过、1 项条件跳过，manifest、零 lint 警告、全部类型检查、最终构建与 33 个 dry-run pack 均成功。本批证据见[npm 生命周期证据](evidence/npm-lifecycle-2026-09-09.json)，上一批离线事务证据仍独立保留。npm 资产尚未正式发布，Lark/supervised 常驻 service lifecycle 等仍未完成，因此 WP17 保持实现中；全部 18 项仍为 **3 已验证 / 13 实现中 / 2 待做**。
