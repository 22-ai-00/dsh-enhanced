...
当前交付主线集中为“owner 授权等待事件 → 重启恢复 → 同一 Session/Goal 继续 → 实际产出 → 独立验收 → 结果投递”。同一时间只推进一个主交付能力，最多保留一个独立辅助任务；并行按这条流程的接口分工，尽早运行真实集成。开发中使用定向检查，代码冻结后集中执行全仓检查与安装验证。评测报表和计量精细化只在影响当前正确性时进入本批。随后按主动处理、技能复用、持续改进的完整使用流程推进，全部 18 项范围不变；进度以新增可交付用户能力衡量。
