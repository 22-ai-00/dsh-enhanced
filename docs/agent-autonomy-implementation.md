...
> 最新实施批次（2026-09-09，基线 `09b4502`）：增加 exact Lark Calendar 只读来源，复用连接器凭据生命周期、有限观察、持久事件和撤权边界；Skills 增加 `amountCents` v2、符号链接 CLI 入口与只读状态来源兼容。真实模型已通过初次捕获、候选复用和启用，但后续已有文件复查被 `FS_NOT_OBSERVED` 阻止，完整 canary 尚未通过；真实 Calendar/GitHub 尚未验收。根 `pnpm check` 通过：350 文件、4,257 项测试、零跳过、33 份打包；独立复核 PASS。工程检查结果与失败材料见[本批证据](evidence/calendar-and-skill-runtime-2026-09-09.json)。全部 18 项仍为 **3 已验证 / 13 实现中 / 2 待做**；之前已交付的有限 canary 机制见[上批证据](evidence/prospective-skill-canary-2026-09-09.json)。
