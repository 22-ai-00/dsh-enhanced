...
> 2026-09-12 真实修复序列冷恢复（基线 `261852d`）：独立 TraeX/Web 完整 E2E 已退出 0（6.1 分钟）。首轮原生修复 Goal 独立完成、模型与工具均结算后 SIGKILL，重启沿原 Session/Goal 继续捕获、比较、实际 canary 与第二轮改进；最终 v1→v2→v3，修复累计 **8 次模型 / 6 次工具调用**，5 条反馈各接受一次，完成后再次重启不重放。首版运行实际发现 Isolation 旧控制器租约阻断 Host 启动，已改为有界等待自然到期，并在启用 Isolation 的 repair 部署中增加 Skills 就绪依赖；审查另要求启动等待期间保持原权限拦截。真实命令与失败记录见[本批证据](evidence/real-repair-checkpoint-2026-09-12.json)；最终根 `CI=true pnpm check` 退出 0：**5,294 passed / 42 skipped、33 份 dry-run pack**，9 个源码与测试文件的冻结哈希一致。启动权限拦截、依赖激活、31 秒有界等待和卸载取消均有通过的行为回归；独立审查 PASS，范围与限制见同一证据记录。本次 SIGKILL 时原生 Goal 已完成，不证明未完成原生 Goal 或未结算外部调用的真实恢复。增量仍未发布 NPM，完整 18 项保持 **3 已验证 / 13 实现中 / 2 待做**。

> 2026-09-12 修复检查点恢复（基线 `95b60d6`）：dev 新增每轮修复的持久执行凭证；Linux 同一 PID namespace 内确认旧进程退出、模型与工具无未结算调用后，可续接原 Session/Goal，保留累计调用、原生轮次与原截止时间。工具结果必须落入 Session 日志；人工暂停、撤权、期限/目标变化以及未确认外部工作继续阻止自动恢复。真实双 Node 进程测试使用确定性模型适配器，首轮执行与独立验收结算后 SIGKILL，再由第二进程完成同一目标、实际写产物，累计 4 次模型/2 次工具调用、两轮及原期限均保持；存活旧进程拒绝竞争接管。独立审查及最终 `CI=true pnpm check` 均通过：**5,285 passed / 42 skipped、33 份 dry-run pack**；新增验收队列合并时序与取消后未结算保护回归。源码及测试冻结哈希一致，完整结果见[本批证据](evidence/repair-checkpoint-recovery-2026-09-12.json)。尚未验证真实模型中断恢复、完整生产 Skills 服务到反馈链路或任意外部子进程清理；此增量尚未发布 NPM。完整 18 项保持 **3 已验证 / 13 实现中 / 2 待做**。

> 2026-09-12 npm 升级闭环：候选 v4 已在独立 Linux `autonomy` profile 完成真实 `.30 → .31` 升级；补齐预检约束后的最终冻结候选 v6 已完成 `.31 → .31` 重复升级，尚未用最终源码重跑跨版本升级。最终锁文件在私有目录校验并完整预取，事务内无网络物化，实际 Host 启动及原子提交通过；14 个受管包均为 `.31`，选定配置/状态文件以及通过 GoalStore API 预先保存的暂停目标和检查点保持不变。此前三次失败均经正式恢复保留证据；修复了临时工作区可见性和 pnpm 扫描旧 Host peer 外链产生额外 bin 的问题。pnpm bootstrap 配置预检与 importer 双向比较已补齐并通过独立审查；34 项 npm 升级定向测试和 14 项元数据测试通过，最终冻结候选同版本重复升级与状态回读也通过。另修复本地依赖准备失败仍继续升级的问题，3 项回归测试证明旧代码会继续停服、现代码按原退出码在事务前终止。最终 `CI=true pnpm check` 退出 0：**5,264 passed / 42 skipped、33 份 dry-run pack**，冻结源码和测试哈希保持一致。该源码修复尚未发布为新 NPM 版本，不代表旧远程 `v0.1.31` helper 已修复，也不证明 in-flight repair 恢复。见[完整升级记录](evidence/lifecycle-npm-prepared-upgrade-2026-09-12.json)；前一批检查与原型记录保留于[预检记录](evidence/lifecycle-npm-upgrade-preflight-2026-09-12.json)。WP17 仍为实现中，完整 18 项保持 **3 已验证 / 13 实现中 / 2 待做**。
...
> 第二类真实 RSI 任务与新安装修复（2026-09-12，源码 `ae8cb4c`）：TraeX 拓扑排序任务完整 E2E 退出 0（3.8 分钟），完成 v1 真实失败、独立 native repair Goal、自主生成修复、前瞻比较、有限 canary、后续真实任务晋升到 v2、原会话反馈及完成后重启不重放；修复账本为 4 次模型调用 / 3 次工具调用。该任务族本次一轮，此前模板任务的两轮组合证据另计；未提供修复源码，未证明修复中断后的恢复或生产 holdout 隔离。新 profile 安装器在 setup 前调用精确 DSH 的公开依赖准备接口，不启动 Host；正式 NPM 0.1.30 包已在全新 Linux profile 通过安装、14 包同版本、全部插件激活和实际 Docker 隔离诊断，修复已进入 0.1.31 发布候选。见[拓扑任务证据](evidence/topology-real-repair-2026-09-12.json)与[新安装证据](evidence/autonomy-install-host-fallback-2026-09-12.json)。仍需 3–5 类真实工作流、通用收益、in-flight 安全重新获取及全部后续规划；18 项状态保持 **3 已验证 / 13 实现中 / 2 待做**。

> 基本 RSI 首版交付（2026-09-12，基线 `8612a35`）：正式 repair admission 冻结有限两轮 profiles、来源/owner lineage、route、期限与反馈权限。真实 TraeX 模板渲染流程一次 arm 后自主完成 v1→v2→v3，两轮独立 Goal/留出比较及后续真实任务晋升通过；修复累计 8 次模型调用/6 次工具调用，小于冻结 16/16 上限。原 E2E 退出 1，仅最终 UI 导航断言失败；修正实际会话选择与新 follow 检查后，独立验证者在同一保留状态启动 Host/Chromium，确认原会话最终通知可见、5 条通知各接受一次、5 个 tick 的状态/预算/通知不变、重启未新增模型调用，补验退出 0。根 `pnpm check` 退出 0：5,239 passed / 1 skipped、33 份 dry-run pack。此为组合验收，不冒充修正后整条模型 E2E 命令重新通过。in-flight repair 重启仍转 `unknown` 且不自动重放；通用增益、其余真实任务及完整 RSI 规划继续保留。全部 18 项仍为 **3 已验证 / 13 实现中 / 2 待做**。见[交付证据](evidence/basic-rsi-two-round-2026-09-12.json)。

> External broker 验收产物交付（2026-09-12，基线 `7a36512`）：正式 Goal admission 可引用 operator 已配置且完全匹配的 `externalGrantId`，继承有限调度、原 owner/session 及反馈权限。独立验收后的产物经签名 UDS 提交，再按显式授权创建 PR；Host 不取得 GitHub token，仅持久化无产物 bytes 的签名回执。repository goal readback 重建并验证历史回执，再读取当前 PR、branch、checks、reviews，跨 broker generation 或 policy/emergency epoch 的观察不能通过。账本 schema 2 原子迁移保留旧 grant/history/unknown 和额度；同目标未决写入不得换 key 重放。外部 branch 创建、补偿、事件轮询、live GitHub 与独立 UID 部署仍是后续工作，不阻塞已交付的有界两轮 RSI 首版，也不改变完整 18 项状态。定向组合通过 61 项，检查通过 5,268 项 / 1 skipped、33 包；真实 Web/TraeX 完整流程退出 0（2.8 分钟），实际完成独立验收、签名 commit/PR、原会话通知及重启可见性。后续工具回执脱敏修复另通过 36 项及该完整 E2E，精确发布提交另行执行根检查。完整证据见[本批记录](evidence/external-verified-delivery-2026-09-12.json)。

> External broker PR 跟进接线（2026-09-12，基线 `0cff080`）：外部代理新增显式授权的 PR/checks/reviews 只读观察；grant、Host projection、签名 request 与账本精确绑定 repository/head/base，PR 必须提供编号，旧 grant 不自动获得新权限。PR 读取计 1 个 API unit，checks/reviews 计 2 个，累计额度与重启去重由代理账本持久执行。签名 UDS→生产 Core/SQLite→protected test token 的集成覆盖三次读取、超预算派发前拒绝与重启同 key 不重放；GitHub transport 和 peer inspector 是明确 fixture，未证明 live GitHub 或独立 UID 部署。外部 branch/PR 创建、补偿、verified delivery 与 repository goal readback 继续保留为后续缺口，全部 18 项状态不变。最终检查结果与源码哈希见[本批证据](evidence/external-pr-inspection-2026-09-12.json)。

> 最新 WP04 independent holdout provider 工程切片（2026-09-12，基线 `89751c3`；最终根检查退出 0，5,154 passed / 1 skipped，33 份 dry-run pack；日志 SHA-256：`a4362f9da0e7d3625c6949949af36a9069e147c75ff69322938b44ffeb92aa3e`，见[完整证据](evidence/wp04-independent-holdout-provider-2026-09-12.json)）：Evaluation 新增 inert `./benchmark/holdout` Host SDK；import 和 plain SDK 不会自动挂载 Cordis、开库、spawn authority 或注册模型工具。插件内入口 `runIndependentHoldoutInContext()` 把 journal/evidence descriptors、authority/delegate session 和取消控制放入调用方当前 Fiber 的 owned effect，卸载会 abort 并等待有界清理，自然完成也关闭 stores 并移除 effect；这只是 Cordis 生命周期所有权，不是 OS 隔离。operator 固定 exact dataset pin 与 Ed25519 key，Host 严格验证 signed manifest/input/verdict/finish：manifest 只公开 case identity 与 input/acceptance commitments，input 绑定临时私有 bytes，verdict 绑定 exact output digest，finish 绑定完整 ordered signed-verdict chain。Host 自行核对冻结 versions、预算、metrics、execution evidence 与 quiescence；authority 不提供计量，当前也没有独立外部 meter。SQLite/evidence/report 不保存 signed input envelope 或 raw input/output，Host 结算后尽力清零 byte arrays；这不能约束 provider/delegate/模型端或同 UID 进程另行复制。完整 journal+completion marker 可零 spawn 离线重验；unknown、partial prefix、stale intent、缺失或漂移 marker 均不续跑。最后一格的 finish marker 先于 SQLite cell 终态，二者间真实崩溃会保留 running intent，operator 必须确认旧执行停止后显式 interrupt 为 unknown，不能由 marker 自动补成功。provider 固定 `cwd=/`、使用显式 environment 且有界关闭，但 executable pathname 未 fd/inode 绑定。当前真实子进程只运行 same-UID synthetic fixture，不证明独立部署；签名/digest 没有历史冻结透明 anchor，`acceptanceDigest` 只是 commitment 而非保密。尚无真实模型、真实独立 holdout 或 live benefit 证据。WP04 继续**实现中**，全部 18 项仍为 **3 已验证 / 13 实现中 / 2 待做**。

> External broker 冻结验证：根测试为 32 files / 492 passed / 1 skipped；根与递归 aggregate 为 359 passed files / 12 skipped files、5,052 passed / 43 skipped tests，33 份 dry-run pack 完成。最终根日志 `/tmp/dsh-external-broker-root-check-final-v3.log` 的 SHA-256 为 `1f21d53de9afaac1d3fbf7fb9ef571f80c2b68314c216cfdd55b814a2c60a74f`。

> 最新 external GitHub broker 工程切片（2026-09-12，基线 `7c9f9e6`）：WP08–10 在同一个 `@dsh-enhanced/assistant-actions` 发布包内保留 Cordis Host facade，并新增由 operator/service manager 独立启动的 `dsh-actions-broker`；profile patch 只挂 Host，不自动 spawn daemon。`external-unix-v1` 把 authoritative grant/tombstone、Linux protected-file token、WAL/FULL journal、Policy/emergency epoch 和 GitHub HTTPS 收进 daemon，Host 只持只读 grant projection 并通过分别限容的 action/admin Unix socket 请求。两端均核对 socket inode/owner/mode 与 Linux `SO_PEERCRED`，server hello/request/response/admin wire 由 pinned Ed25519 identity 绑定 generation、challenge、owner/Session、source/destination、grant/payload、期限与预算；action/admin listener 拒绝另一 audience 的 frame，action connection flood 不占 admin listener capacity。首期只开放 expected-head commit 与 repository/branch/allowed-file inspect；PR/check/review inspect 及 branch/PR create、compensation/rollback、verified delivery、repository readback 在 external 模式 fail closed 且不回退 `embedded-compat`。实际配置 token 在预期独立 UID 部署中只由 daemon protected file 读取、不进入 Host；wire schema 拒绝 credential 命名字段，daemon 在 GitHub 出站和 inspect response 放行前另以全部配置 token 精确值及常见 credential literal 扫描，但这不是通用语义 DLP。daemon recovery 将未派发 prepared 终结为 failed，将可能已派发的 crash/lost-ACK 保留 unknown/no-replay；stop/resume 单调推进 emergency epoch，stop/revoke 持久终结准入并 abort in-flight，但不能撤销 GitHub 已接受的请求。Cordis external service 不需要 Keychain，卸载会 abort 并等待本 fiber 的在途 IPC；embedded 模式则继续随 Keychain provider 出现、卸载与替换。Linux peer inspector 当前依赖 pinned Node 的私有 `net.Socket._handle.fd` 和 koffi/libc `SO_PEERCRED` ABI，缺失时 fail closed，升级 Node/koffi/libc 后必须用 built real UDS 重新验收。built subprocess E2E 两项均使用真实子进程、双 UDS、SQLite、Ed25519 与 protected token：production CLI 场景验证 status、daemon-side DLP 拒绝与 stop；fixture child 场景用可控测试 transport 在 dispatch 后 SIGKILL，重启相同 semantic key 得到 unknown 且 transport count 保持 1。两者仍是 same-UID，第二项也不是 production CLI transport；均未请求 live GitHub，不能证明独立 UID、生产凭据或远端原子性。strict DTO 与 token-pattern/equality 检查不是通用语义 DLP，也没有外部单调 audit anchor、systemd provisioning 或独立 UID 实跑。最终 Actions 17 files / 261 tests、Keychain 6 files / 28 tests、built subprocess 2/2 及 package build/pack 已通过；独立安全与 Cordis lifecycle review 均为 0 BLOCK / 0 HIGH / 0 MEDIUM。根 `CI=true VITEST_MAX_WORKERS=1 pnpm check` 退出 0：root 32 files / 492 passed / 1 skipped，递归 327 passed files / 12 skipped files、4,560 passed / 42 skipped tests，合计 5,052 passed / 43 skipped，33 份 dry-run pack 完成。WP08、WP09、WP10 均保持**实现中**，全部 18 项仍为 **3 已验证 / 13 实现中 / 2 待做**。完整命令、日志、冻结源码哈希和限制见[结构化记录](evidence/external-github-broker-2026-09-12.json)。
...
## 2026-09-13：WP15 四 cohort deployment cohort watch 关闭

基线 `5d6ad54`。推广后质量监控从 qualified deterministic skill family 扩展到原始条款要求的全部四类部署，每类的退化自动关闭/回滚、撤票回滚、正向证据不误撤与重启继续观察均落在真实执行的工程层测试上。

guidance cohort：EvolutionStore 在写事务内把 approved adopt proposal 冻结的 `evidence.taskRevisions` 元组与 active 规则做 SQL JOIN 投影；冻结 subject 的 upsert 纠正或纯撤票都以 `WHERE id=? AND status='active' AND version=?` CAS 精确退休该规则版本一次并写 `evidence-correction-rollback` 审计（每规则仅一次），正向证据、异 situation/异 subject kind、非冻结 subject withdraw 均不 churn；两个测试均含 close/reopen 回读（9157dc8 纠正用例与 5d6ad54 纯 retract 用例）。该 cohort 无版本化部署物，"回滚"语义即依赖冻结 taskRevision 的 active 规则退休 + 审计，不与 plugin 的账本终态混用。

skill 与 workflow cohort：verified-workflow repair source 的定义只能经 createDefinition（强制 `assistant-goals/verified-workflow-source/v1`，全包无其他 SkillDefinition 构造路径）通过 SkillStore.save 落库，因此 canary、promotion、canonical replacement、exact rollback 与重启测试（assistant-skills 整包 318 passed / 11 skipped）全部实际跑在 workflow-sourced 定义上；canonical revision 2 not-achieved 与 invalidated retract 均在 Evaluation writer fence 内与 deployment transition、exact parent rollback 单事务提交，stale revision 不覆盖、同 revision 分歧拒绝，历史证据 skill-canonical-revision-monitor-2026-09-11 与 repeated-failure-topology-canary-2026-09-10 继续有效。scope 明确不声称 verified-workflow source 之外导入的 workflow。

plugin cohort：plugin-control-plane schema v12 在 activation 落库时自动以 exact package/version/integrity 三元组开 activation_watch（revision 1、watching）。Host 根（Ed25519 host-runtime/host-key-1）签名观察按严格递增 host generation 推进，regressed 经 state+revision CAS 关闭为 closed-regressed 并拒绝后续证据，healthy 仅累加计数；独立 owner 根（owner-policy/owner-key-1）签名撤票关闭为 closed-retracted、删除 gap_plan_claims 并重开 capability_gap（open、candidate_id 清空、revision+1），watching 与 closed-regressed 态均可撤票、重复撤票拒绝。观察绑定 installationId/planId/planDigest/activationId/fence 与 exact 三元组，异根互签、篡改、超 TTL（30000/900000）、stale generation/revision、disposition 与 failures 矛盾等 19 个收据级反例被拒；operation_receipts 保证幂等与历史快照回放。state.spec.ts watch describe 共 9 个测试（含 v11→v12 从 candidate_json 不伪造证据回填 watch 并继续观察），cli.spec.ts 3 个真实子进程 e2e（healthy+regressed 关闭、撤票 gap 重开、错误 exact 版本在验签前拒绝），post-activation.spec.ts 20 个收据级测试；plugin-control-plane 全包 187 passed。物理边界诚实：dsh executor 以 trust.json 钉 sha256 且 `dsh plugin` 仅 add，无 forced uninstall/disable；"关闭/回滚"即控制面账本权威终态 + gap 重开 + 经不可变 catalog package@version 重新激活，测试显式断言退化/撤票后 plan 仍 activated。

消费侧：assistant-health 把 watchingActivations/closedRegressed/closedRetracted 接为 optional-number 指标，closed* 仅派生 `provider-degraded` 警告（不阻断 readiness），pre-v12 五字段 payload 继续合法；assistant-health 全包 51 passed。

独立对抗式只读复核为 **PASS：0 BLOCK / 0 HIGH / 2 MEDIUM（矩阵行与证据未提交改动，均在本关闭提交内解决）**，另记录 4 个 low：workflow cohort 无独立测试身份（结构守卫当前成立，若开放 verified-workflow source 之外导入路径需补独立守卫测试）、watch describe 实际 9 个测试（已订正）、holdout-authority.spec.ts 默认并发下 1 次 flake（单 worker 与单跑均绿，属 WP14 既有并行敏感，与 WP15 无关）、生产 cohort/多 cohort 并行/长期分布/物理强制停用按原始边界延后。最终根 `CI=true VITEST_MAX_WORKERS=1 pnpm check` 退出 0：根测试 37 files / 564 passed / 2 skipped，递归聚合 389 passed files / 12 skipped files、5,474 passed / 44 skipped，33 份 dry-run pack 完成；日志 `/tmp/dsh-wp15-cohort-coverage-final.log` 的 SHA-256 为 `94195e63dc49891369654879eb185f391e5c04217b3b74ed48f415a6da36b442`。12 个冻结源码/测试文件哈希与证据清单逐一吻合。WP15 按原始条款转为**已验证**，完整工作包现在为 **5 已验证 / 11 实现中 / 2 待做**；真实生产 cohort、多 cohort 并行与长期质量分布为后续增强，不反向构成首版门槛。结构化证据见[四 cohort cohort-watch 记录](evidence/plugin-deployment-cohort-watch-2026-09-13.json)。
