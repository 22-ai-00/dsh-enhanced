...
- Codex CLI `0.147.0`：`coding-subscription-provider` 默认 `transport: cli`；App Server 的 `initialize` + `model/list` 用于无 prompt 模型目录发现，生成走 JSONL。Agent 工具由兼容 prompt 中的 `dsh-tool-calls/v1` 严格信封桥接回 DSH，CLI 自身不执行这些工具。生成正文不进入 argv：Codex、Claude 与 Cursor 使用 stdin；Grok POSIX 使用 `--prompt-file /dev/stdin`，Windows 使用 OS 用户临时目录内独占创建的私有文件、继承该用户的 Windows ACL，并在子进程关闭后清理。默认 `maxPromptBytes` 为 4 MiB，是本地序列化/输入内存安全边界；超限返回 `CLI_PROMPT_LIMIT`，与模型 `CONTEXT_WINDOW_EXCEEDED` 分离。工作目录诊断仅在进程以 code `1` 退出、没有 stdout 协议输出，且 stderr 含完整行 `Not inside a trusted directory and --skip-git-repo-check was not specified.`（可伴随 stdin 提示）时专门映射，其他退出保持通用分类。
