...
- Evaluation `./benchmark/skills` 新增可选 Host peers `assistant-skills >=0.1.33 <0.2.0` 与 `dsh-session-query >=0.1.2-rc.1 <0.2.0`。委派 `skill_run`/`skill_status` 的预授权需要匹配的 Skills/Policy/Isolation 构建；缺少版本化能力接口的旧 Policy 不能授予隔离委派。必须保留 live 来源、原始已验收 Session/Goal、精确 owner scope 与最终 prospective receipt；定义 JSON 不能重建委派。升级须验证来源捕获、新 Goal 复用、取消及晚到清理、冻结后签名比较，见[原生技能复用](native-skill-reuse.md)。
- Goals 的 `goal_checkpoint` 精确预授权要求 Policy 提供独立的 `checkpointToolPreauthorizationVersion() === 1`；已有 context capability 版本不变，旧 Policy 不注册此预授权。匹配构建仍须检查 owner、当前 Session/native Goal、checkpoint/observe 权限、记录版本及同 owner 依赖图，执行时重新验证并进行事务 CAS。升级须覆盖正常原生工具调用、撤权、跨 Session、取消、旧版本和无写入预检；失败的历史来源不会因此变成有效技能证据。
...
Growth `usageLearning` uses same-release Evaluation schema 12 canonical feed/writer fences, Delivery ordinary foreground execution/model snapshots introduced in schema 23 (retained in schema 24), owner source inspection, and native Automations Host executors. Its peer floor for those services is `0.1.33`; earlier development builds with that version still require the exact APIs. Evaluation/Automations are late-bound optional peers, not auto-enabled bundles. Delivery captures `request/header` after pinned DSH `llm.prepareCall()` applies defaults; proposed `agent/request` options are insufficient. Schema 22→23 adds a separate execution table without backfilling old ordinary tasks or changing acceptance contracts; Web Owner doctor now pins schema 24. Typed owner feedback can establish an ordinary foreground objective without a preconfigured acceptance profile. Old accepted executions retain their snapshots (or missing state), never a guessed current route. Upgrade checks cover migration, adapter defaults, model changes, native scheduling, correction/retraction and interrupted-job recovery.

### Natural owner feedback (Delivery schema 24)

Schema 23→24 adds a natural-feedback journal without synthesizing votes from older Inbox rows. Explicit natural replies to completed ordinary foreground results can initialize, correct or withdraw the canonical owner judgement while the same message continues into the native Agent. Each journal entry retains the exact original message, reply target, owner lineage, admission order and frozen version/status precondition; recovery only resumes ledger work. Evaluation remains schema 12 and adds an optional `operationId` to its current-owner-revision inspector so Delivery can resolve the exact feedback prose. Growth includes that bounded prose as untrusted review input. Update these bundles together to obtain the complete behavior; the native DSH/Cordis baseline is unchanged.

The Web Owner read-only doctor is reviewed against schema 24. The Lark owner snapshot reader accepts the identical owner tables at schemas 23 and 24 and reports the actual version without migrating the file; this lets staged upgrade preparation inspect the old database before the new Host starts. Service lifecycle transactions accept both versions of that unchanged snapshot protocol, retaining prior-version evidence during upgrade/recovery; this does not allow arbitrary newer schemas. Natural replies do not become replay-safe slash commands. Evaluation absence must not block ordinary conversation, and feedback recovery must never redispatch the model. Unknown foreground execution remains ineligible. Automation, whole-goal and native Web feedback retain their existing entry points.
...
### Trusted task observations (Control Plane schema 21)

Schema 21 adds immutable task observation batches and one open batch per owner/profile lane; schema 20 deployment witnesses and all earlier source-adoption records are preserved. No historical batch is backfilled. Older control-plane binaries cannot open this ledger. The optional `taskObservations` configuration requires the same release's Evaluation `getTrustedForegroundLearningProjection` and canonical writer fence, Delivery owner-source API, Automations Host executor/reconciliation, and foreground deployment witnesses. Evaluation's exact lookup adds no database migration.

The native cron freezes distinct current task revisions, requests a finite owner-policy signature, and fences the final watch/batch transaction after asynchronous verification. Same task revisions are deduplicated across the complete applied history. Disposal pauses only the owning runtime generation, aborts and drains its subprocess/rollback work; a fresh generation reactivates persisted scanning. Pending batches can resume, stale/expired evidence cannot be refreshed under the same identity, and committed rollback obligations survive feedback withdrawal. This is a deployment cohort policy, not proof of causal tool use or production quality. Same-UID private signer files remain within the documented Host trust boundary.


### Cross-Host adoption handoff (Control Plane schema 23)

Schema 23 adds target-created, non-renewable adoption handoffs; existing plans receive no delegated authority. Optional `dossier.handoff` terms are included in the immutable activation digest and must exactly match the source adoption signer's `grant.handoff`. Existing schema-4 plans without terms retain their digest. Older strict readers do not understand the new dossier field; stop old writers and upgrade both Hosts and the signer together. Schema-22 binaries reject this database.

A dedicated coordinator connection accepts only a live, exact source-linked handoff for forward deployment, stopping before target-owned final commit. Target continuation checks live owner feedback before commit; rollback retains the existing activation fence and unknown-dispatch reconciliation. `probePluginPlan({ deferCommit: true })` leaves successful health at `commit-pending`, while normal callers retain automatic commit. The coordinator reuses native Automations rather than a separate scheduler. It must run outside the restarted target unit, with matching protected ledger/trust; shared UID file access and the bounded offline exposure window are documented in the [plugin guide](../plugins/plugin-control-plane/README.md#已发布-owner-修复的有限采用). Installer wiring and independent behavioral attestations remain release gates.
