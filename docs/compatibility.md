...
- `@deepseek-ai/dsh-permission-presets@0.1.0-rc.8`、`@deepseek-ai/dsh-sandbox-policy@0.1.0-rc.8` 与 `@deepseek-ai/dsh-user-approval@0.1.0-rc.8`：三档控制复用上游 session 事实源和 canonical setters，映射为 `workspace-write + ask + user`、`workspace-write + ask + auto-review`、`danger-full-access + never + none`。切换命令会把 sandbox/approval 显式物化到 session，不能依赖部署默认值。Direct Codex、TraeX 及其他 provider 都只生成 tool call，最终权限由同一个 DSH ToolRuntime / approval / sandbox / AssistantPolicy 管线执行；私有 Responses transport 不继承 Codex CLI/App Server 自带的 sandbox 或 Guardian。
