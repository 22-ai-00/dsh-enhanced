...
- pnpm：`11.7.0`
- `0.1.4` 配置迁移：`assistant-delivery` 与 `assistant-automations` 已废弃 `toolCapableProviders`、`unknownRouteToolCalls`。旧键会被配置解析器保留但运行时忽略，升级时应删除；工具集合现在只由 Agent/preset 决定，route 缺少协议元数据以及声明 `native`/`bridge` 均准入，只有 adapter 明确声明 `toolCalls: none` 且最终工具非空时因协议未实现而失败。
- Codex CLI `0.147.0`：`coding-subscription-provider` 默认 `transport: cli`；App Server 的 `initialize` + `model/list` 用于无 prompt 模型目录发现，生成走 JSONL。Agent 工具由兼容 prompt 中的 `dsh-tool-calls/v1` 严格信封桥接回 DSH，CLI 自身不执行这些工具。工作目录诊断仅在进程以 code `1` 退出、没有 stdout 协议输出，且 stderr 含完整行 `Not inside a trusted directory and --skip-git-repo-check was not specified.`（可伴随 stdin 提示）时专门映射，其他退出保持通用分类。
...
- `@deepseek-ai/dsh-agent@0.1.0-rc.8` 的公开 initiator scope 覆盖完整 driver Promise。两个本机 provider 用它跨 `dsh-llm` 包副本和 `forAdapter()` 请求克隆恢复来源证明，但不会把 ambient initiator 本身当作授权：必须同时满足 running/registry/Session 对象身份，从 live Session 重建普通对话 header/messages，并且只容许 rc.8 对跨 route 历史删除 `replayState` 的已知变换。bundle 默认把 provider `cwd` 与标准 Delivery 的 `$DSH_HOME/assistant-workspace` 对齐；手工整段覆盖 config 时必须保持两端 canonical realpath 一致。
