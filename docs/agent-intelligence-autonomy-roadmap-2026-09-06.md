...
按用户 2026-09-07 调整，阶段按依赖关系和验收证据连续推进；此前按天、周列出的安排只是人工工作量估算，不是执行配额、最低耗时或等待条件。独立任务可并行，前置条件完成即可进入下一阶段，不为日历排期停工或拖延交付。功能本身的超时、冷却、租约、调度和恢复时间语义仍需验证，适合的场景可用受控时钟加速，并明确区分加速验证与真实长期使用观察。

| 阶段 | 工作与仓库落点 | 建议验收门 |
| --- | --- | --- |
| P0：可信基线 | 修复 Canary canonical projection/fence；升级 replay/shadow 验证；对齐安装文档；在 `assistant-evaluation` 接入第一批领域 verifier | 冲突/撤回样本绝不晋升；区分执行/目标/送达；固定任务集得到可重复基线；现有 check 与新反例通过 |
| P1：自主完成任务 | 在原生 goal 上加业务目标编排插件；改进 Memory 任务召回；接通任务结果和重规划；复用 Delivery/Automations 恢复 | 多步骤真实任务能跨会话/重启恢复；无重复外部提交；同模型同预算比基线有稳定改善 |
| P2：高权限执行，可与 P1 并行 | 扩展 Policy 为授权 lease；凭据/动作 broker；隔离 worker；独立 audit/stop；优先完成一个生产平台 | worker 无法访问信任根和通用 token；所有实际副作用经过 broker；子进程/网络绕行、撤销竞态、进程崩溃反例被拦截 |
| P3：有价值的主动性 | 在 event-triggers/Heartbeat 上加事件到目标的关联和机会排序；先接代码/任务/日历中的两个来源 | 提醒被采纳率、漏掉可行动机会、每周打扰次数和主动收益均可测；交付前验证功能、指标记录与安全边界，交付后在实际使用中持续观察和打磨，不设固定观察期 |
| P4：自主策略与技能进化 | 扩展 Growth 的 typed workflow、真实基线对比和留出集；技能实验；沿 Control Plane 补所需生产 adapter | 相同预算下留出集表现改善，重要回归集不过线则不晋升；晋升后退化自动关闭对应版本；能重放完整证据链 |
...
可先把“常规授权内任务免逐动作审批”“成本受限且留出集有增益”设为产品目标。主动性长期效果在交付后的真实使用中持续观察和打磨，不作为固定时长的交付门槛。不要宣称有限测试中的零越界等于绝对安全。
