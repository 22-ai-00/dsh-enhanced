...
A `complete` comparison means observations are complete, not that the candidate is better. Quality fields separately report all candidate checks, evaluation success-rate difference and critical regressions. Paired observations and bootstrap statistics come from the existing Evaluation benchmark implementation; repeated deterministic trials are not independent tasks. The report always sets `promotionAuthorized: false` and `heldoutIndependence: unproven`. A Host-attested plan may add a binding digest to the receipt, but that legacy binding has no production provider; the separate [external authority entrypoint](skill-holdout-authority.md) supplies independently signed judgments through skill_qualify; that field is not evidence of OS sealing, historical independence, or model inaccessibility. Cases are kept out of replay workers and tool responses, but the calling Host Agent may have broader filesystem authority. Automatic promotion still requires an independently established boundary, independent held-out gains and a separately authorized deployment transition. Owner-requested activation after a fresh Goal trial remains a separate operation.
