...
`inputs` contains only declared skill parameters; `files` provides bounded initial text snapshots as `{path, content}`. Both arms receive the same values. File paths must be relative without traversal, or inside the workflow's original source workspace; source paths are rebased into the private temporary workspace. The comparator executes fixed read/write/edit steps. Strictly validated workspace-local glob/grep and parameterless get_goal are retained as provenance-only observations: they are not executed and no output is fabricated. All source calls still consume the declared tool ceiling, while executedToolCalls and omittedObservations separately report actual replay. Other tools, escalation arguments, external paths and oversized inputs are rejected; it does not silently reinterpret an arbitrary workflow as a file workflow. Initial snapshots are operator supplied; their historical provenance is not automatically attested.
...
A `complete` comparison means observations are complete, not that the candidate is better. Quality fields separately report all candidate checks, evaluation success-rate difference and critical regressions. Paired observations and bootstrap statistics come from the existing Evaluation benchmark implementation; repeated deterministic trials are not independent tasks. The local comparator always sets `promotionAuthorized: false` and `heldoutIndependence: unproven`; so does every external qualification against an operator-pinned dataset digest, because pre-staged cases may already have leaked and the legacy Host-attested plan binding has no production provider.

The prospective external authority path (`skill_qualify` with a pinned `generatorDigest`, described in the [external authority entrypoint](skill-holdout-authority.md)) is the only path that can set `heldoutIndependence` to `attested-after-freeze`, and only on a complete receipt with all three quality gates passing. That value means exactly this: the frozen binding (scope, both definition digests, budget, limits and any canary admission) was signed before the authority's fixed-semantics CSPRNG generated the private cases, the generator semantics are pinned by digest, the generated dataset digest is signed into the certificate, and the pinned authority signs each cell's private input and judges every observation, so neither arm's deterministic replay could have read its answers ahead of time. The certificate is a cryptographic generation-order claim, not an isolation claim: it does not attest process separation, that Docker actually applied its sandbox flags, live filesystem permissions, OS or kernel isolation, the candidate model's training history, or the dockerd trust root. Zero side effects during comparison are carried by separate facts — `modelCalls: 0`, fixed native file-tool replay, and the runner's no-network, read-only, capability-dropped container with a private state root separated from the workspace — not by `heldoutIndependence`. Cases are kept out of replay workers and tool responses, but the calling Host Agent may have broader filesystem authority. `promotionAuthorized` stays false on every path. Automatic promotion still requires the separately authorized canary deployment transition; owner-requested activation after a fresh Goal trial remains a separate operation.
