...
> 2026-09-08 模型准备批次（基线 `21ccaf9`）：显式 `prepare.preparation` 已通过现有真实模型线路完成持久草稿；独立 Session、空工具面、单次请求、固定主体预算和当前目标/主人/来源授权检查均接通。真实浏览器 5 次 Host 启动、7 次模型请求（4 次前台授权、1 次准备、2 次回读），草稿代码通过三个离线容器检查，重启与重复事件不再生成，原 Web 会话可取回完整代码。通用草稿仍标为未验证，原目标不执行；智能排序、技能复用与持续改进仍须推进。完整 18 项保持 **3 已验证 / 10 实现中 / 5 待做**；最终工程检查和独立结论见[本批证据](evidence/proactive-preparation-2026-09-08.json)。以下为历史记录。

> 2026-09-08 主动提醒批次（基线 `a5ea259`）：`remind` 已接通持久 Delivery 队列与原生 Web 会话提醒区。真实现有线路验证 4 次 Host 启动、原 Session 重启前后提醒可见、重复事件不重复提醒；4 次模型请求均用于前台授权等待，提醒不新增模型请求、不执行目标。发送前重查主人、绑定代次、期限、路由和当前发送 Policy。完整 18 项保持 **3 已验证 / 10 实现中 / 5 待做**；模型准备和智能排序仍待后续交付。最终工程检查与独立验收见[本批证据](evidence/proactive-reminder-2026-09-08.json)。以下为历史记录。

> 2026-09-08 机会筛选批次（基线 `93905e5`）：新增独立 `assistant-proactive`，主人可为目标事件等待显式选择收益筛选、合并、静默与预算策略；执行仍进入原 GoalWake 权限和验收流程。真实现有路线已验证 3 个 Host、7 次模型调用、合并窗口结束后原 Session/Goal 完成、独立验收与 Web 反馈。`prepare` 仅保存结构化记录，提醒发送、模型草稿、智能排序和长期收益仍未交付。完整 18 项为 **3 已验证 / 10 实现中 / 5 待做**。最终工程证据见[本批记录](evidence/proactive-event-2026-09-08.json)。以下为历史交付记录。

> 2026-09-08 最新交付批次（基线 `5923a44`）：现有 `super-relay / auto_model/alwaysday1` 已真实贯通 owner 授权事件等待 → Host 重启后文件变化 → 同一原 Session/Goal 恢复写产物 → 独立步骤/全目标验收 → Web 原回复可见且持久回读，重复事件不再执行。新增显式持久调用次数预算（不承诺 token/金额上限），修复事件预授权导致 Goals 工具整组失效，以及 Web 已有原生回复却重复进入无 adapter Outbox 的缺口。最终检查、真实运行及失败记录见[本批证据](evidence/real-event-goal-2026-09-08.json)。完整 18 项仍为 3 已验证 / 9 实现中 / 6 待做；下一批集中主动处理，再技能复用与持续改进，不设置日历等待。

> 2026-09-08 最新交付批次（基线 `ea107bf`）：真实模型验证已使用现有 `super-relay / auto_model/alwaysday1`，不依赖 DeepSeek 凭据。新增 `goal_create(start_native_rounds: true)`，成功创建后通过原生工具回合结束接口明确交接；缺省行为保留同回合设置事件等待/调度。全新 Web profile 的真实模型 3 次请求完成原生回合写产物、v2 步骤与 v3 全目标独立验收，并通过真实 Host 重启、原 Session/Goal/产物/完整回复不变及无新增请求检查。最终全检和独立复核见[本批证据](evidence/real-gateway-goal-2026-09-08.json)。这是已完成目标的重载验证；真实模型驱动的未完成事件恢复和该网关的生产累计 token 预算仍须接线，18 项继续保持 3 已验证 / 9 实现中 / 6 待做。

> 2026-09-08 事件目标继续执行（基线 `1592f52`）：可选 `goal_wait_event` 已贯通 owner 授权、持久来源序号、原 Session/Goal 恢复、工具生成报告、独立验收及真实结果入队。五项集成含独立 Node 进程重启、目标变更和来源 Policy 撤销；模型与渠道均为明确替身。实现中发现并修复了“目标已恢复但结果未投递”的生产缺口。交付检查与限制见[本批证据](evidence/event-goal-wait-2026-09-08.json)。完整 18 项仍为 **3 已验证 / 9 实现中 / 6 待做**；后续按主动处理、技能复用、持续改进的完整用户流程推进，日历排期和长期观察不设等待门槛。
...
基线 `93905e5` 已推送：真实现有模型路线完成 owner 等待文件事件、Host 重启、原 Session/Goal 恢复、实际产物、独立验收与页面反馈。之后新增 `assistant-proactive` 接线，按主人选择的策略对事件做合并、静默和收益筛选；执行继续复用同一条 GoalWake 链路。当前细节以[实施账本](agent-autonomy-implementation.md)末尾及插件 README 为准。历史 WIP 段落不代表当前状态。

完整计划目前 3 已验证 / 10 实现中 / 5 待做；WP12 尚缺主动提醒、实际模型准备与智能排序，完整目标未完成。每批完成可用能力即验证、提交并交付，不因其余工作包或长期观察推迟已完成结果。

### 2026-09-08 技能复用交付更新

基线 `96e6bf3` 已包含持久主人提醒与真实模型准备草稿。最新一批新增可独立安装的 `assistant-skills`：把独立通过的 Goal 工具轨迹保存为私有版本化技能，重启后经原生目录加载，在新 Goal 中按当前权限执行并重新验收。真实现有模型 Web 流程已通过，回执经原会话工具详情回读；具体最终检查与限制见[技能复用证据](evidence/verified-skill-reuse-2026-09-08.json)及[实施账本](agent-autonomy-implementation.md)末尾。

完整计划更新为 **3 已验证 / 11 实现中 / 4 待做**。本批交付一个完整可用的固定工具技能流程；WP13 的 3–5 类任务覆盖、自动候选、受限试用、推广与回滚仍待后续实现，不把全部规划标为完成。不设置按天进度或两周观察门槛。
