...
The task consumes a JSON array of orders, excludes orders whose `status` is `cancelled`, sums integer `cents` by `currency`, and emits a JSON object with sorted currency keys followed by a newline. The generator uses fresh random values and fixed semantic edge cases across replay, evaluation and regression cases. It does not inspect either program to select favorable questions. Other task domains need their own independently governed judge and are not covered by this generator.
...
On `begin`, the operator first durably freezes the exact scope, baseline, candidate and budget binding with a unique freeze ID. Only then does it generate private cases and sign the certificate binding those cases to that frozen request and generator. A failure after the freeze makes that plan unavailable; restarting cannot draw a replacement dataset. Completed receipts can be reread without rerunning cells. The certificate proves the configured authority's generation sequence; it does not prove training-data history or general intelligence improvement.
...
The authority library and signature helper are available from `@dsh-enhanced/assistant-skills/holdout-authority`. Signatures must be checked against the pinned public key and exact expected qualification identity. Serialized authority state excludes raw cases and private keys. In prospective mode, the private SQLite database additionally retains the generated dataset and certificate for recovery; it must remain outside candidate access.
...
The existing in-process `SealedSkillHoldoutProvider` remains a compatibility binding and does not acquire stronger trust from this CLI. Prospective canary requires the independently configured authority, fair measured comparison, critical regressions, explicit finite deployment authority, and a bound rollback watch. Engineering fixture gains do not establish improvement on real user tasks. No calendar observation period is required, but those functional gates remain mandatory.
