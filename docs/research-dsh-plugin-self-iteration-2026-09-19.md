...
| Controlled rollout and recovery | [completion matrix WP14–WP16](agent-autonomy-completion-matrix-2026-09-11.md); [Recovery](../plugins/assistant-recovery/README.md) | Candidate comparison/canary/watch infrastructure exists; WP16's real remote adapter and WP18's full repository loop remain explicitly incomplete. |
...
The matrix's own evidence boundary matters: local tests, fixtures, a dry-run pack, and a limited real-model run do not establish production autonomy. In particular, WP16 has no real publish/enable/monitor/rollback adapter and WP18 has no complete authorized real-repository evidence chain.
